...
function diagnose(s: SimStats, eff: number | null): Diag[] {
  const out: Diag[] = []
  if (eff != null && eff < 55) {
    out.push({
      tone: 'rose',
      text: `合并效率仅 ${eff.toFixed(0)}%：同 warp 相邻线程没有访问相邻地址，事务数远超理想值。把下标改成 idx = blockIdx.x * blockDim.x + threadIdx.x 这种连续形式，事务数能降到 1/${Math.max(2, Math.round(100 / Math.max(1, eff)))} 左右。`,
    })
  } else if (eff != null && eff < 90) {
    out.push({
      tone: 'amber',
      text: `合并效率 ${eff.toFixed(0)}%：部分访问跨 32B 段。检查索引里是否带 stride、错位或除不尽的偏移。`,
    })
  }
  if (s.bankConflicts > 0) {
    out.push({
      tone: 'amber',
      text: `检测到 ${s.bankConflicts} 次 bank conflict：同 warp 多个线程同拍读写同一 bank 的不同地址，硬件只能串行。常用解法：给 shared 数组做 padding（如把宽 32 改成 33），或调整索引避开 index % 32 撞车。`,
    })
  }
  if (s.divergentBranches > s.warps) {
    out.push({
      tone: 'rose',
      text: `分支分化 ${s.divergentBranches} 次（warp 数 ${s.warps}）：同 warp 线程走了不同路径，两边都要串行跑一遍。尽量让分支条件按 warp 对齐（例如用 threadIdx.x / 32 而不是 threadIdx.x % 2 做条件）。`,
    })
  } else if (s.divergentBranches > 0) {
    out.push({
      tone: 'cyan',
      text: `有 ${s.divergentBranches} 次轻微分支分化 —— 通常来自边界检查的最后一个 warp，无伤大雅。`,
    })
  }
  if (s.sharedWrites > 0 && s.sharedReads > 0 && s.syncBarriers === 0) {
    out.push({
      tone: 'rose',
      text: '写入 shared 后没有任何 __syncthreads() 就读取：结果依赖线程推进时序，换个调度顺序就可能出错。在写与读之间补一道屏障。',
    })
  }
  if (out.length === 0) {
    out.push({
      tone: 'volt',
      text: '访存完美合并、没有 bank conflict、分支整齐 —— 教科书级的 kernel，可以拿去面试现场了。',
    })
  }
  return out
}
...
export function StatsView({ stats }: { stats: SimStats | null }) {
  if (!stats) {
    return (
      <div className="flex flex-col items-center gap-2 py-14 text-center">
        ...
        <p className="text-[13px] text-ink3">还没有统计数据，先 RUN 一次。</p>
      </div>
    )
  }
  ...
  const diags = diagnose(stats, eff)
