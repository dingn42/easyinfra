  ...
  return (
    ...
    >
      <div className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-3">
        ...
        {m.gqaNative && !gqa && (
          <span className="font-mono text-[11px] text-amber">注：LLaMA-2 70B 官方权重原生即 GQA（n_kv=8）</span>
        )}
      </div>

      <div className="mb-5 grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
        <Slider label="序列长度 S" value={seqExp} min={10} max={17} step={0.5}
          onChange={setSeqExp} fmt={(v) => fmtSI(2 ** v, 1)} unit="token" />
        <Slider label="并发 batch" value={batchExp} min={0} max={8} step={1}
          onChange={setBatchExp} fmt={(v) => fmtInt(2 ** v)} unit="请求" />
      </div>
      ...
      <div className="mb-5 rounded-md border border-line bg-panel2/40 p-3">
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-2">
          {factor('2', 'K 和 V')}
          {times}
          {factor(String(m.L), '层数 L')}
          {times}
          {factor(String(nKv), 'KV 头 n_kv', gqa ? 'text-volt' : 'text-ink')}
          {times}
          {factor(String(m.dHead), 'd_head')}
          {times}
          {factor('2 B', 'fp16')}
          <span className="font-mono text-ink3">=</span>
          {factor(fmtBytes(perToken), '每 token', 'text-cyan')}
          {times}
          {factor(fmtSI(S, 1), 'S')}
          <span className="font-mono text-ink3">=</span>
          {factor(fmtBytes(perReq), '每请求', 'text-amber')}
          {times}
          {factor(fmtInt(B), 'batch')}
          <span className="font-mono text-ink3">=</span>
          {factor(fmtBytes(totalKv), 'KV 总量', 'text-amber')}
        </div>
      </div>
      ...
      <div className="mb-5">
        <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
          <span className="microlabel">显存占用（fp16 权重 + KV CACHE）</span>
          <span className="flex items-center gap-3 font-mono text-[10.5px] text-ink3">
            <span><span className="mr-1 inline-block size-2 rounded-sm bg-cyan/70" />权重</span>
        ...
        <div className="relative h-9 overflow-hidden rounded-md border border-line bg-bg2">
          ...
          <div className="absolute inset-y-0 w-px bg-rose shadow-[0_0_8px_rgba(255,92,122,0.8)]" style={{ left: `${linePct}%` }} />
        ...
        <div className="mt-1.5 font-mono text-[11px] tabular-nums text-ink3">
          权重 <span className="text-cyan">{fmtBytes(weights)}</span>
          {' + '}KV <span className="text-amber">{fmtBytes(totalKv)}</span>
          {' = '}<span className={total > HBM ? 'text-rose' : 'text-ink'}>{fmtBytes(total)}</span>
          {total > HBM && <span className="text-rose">（超出 {fmtBytes(total - HBM)}，OOM）</span>}
        </div>
      </div>

      <div className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-4">
        <Stat label="每 TOKEN KV" value={fmtBytes(perToken)} tone="cyan" size="sm" />
        <Stat label="单请求 KV" value={fmtBytes(perReq)} tone="amber" size="sm" />
        <Stat label="KV / 权重之比" value={`${(totalKv / weights).toFixed(2)}×`} size="sm" />
        <Stat label="80GB 最大并发"
          value={maxBatch > 0 ? fmtInt(maxBatch) : '0'}
          unit={maxBatch > 0 ? '请求' : '权重都放不下'}
