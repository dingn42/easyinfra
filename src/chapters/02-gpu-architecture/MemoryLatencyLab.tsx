...
const LEVELS: Level[] = [
  { name: '寄存器', en: 'REG', cycles: 1, cap: '256 KB/SM', tx: 130, ty: 208, tone: 'volt' },
  { name: 'Shared/L1', en: 'SMEM', cycles: 30, cap: '228 KB/SM', tx: 330, ty: 150, tone: 'cyan' },
  { name: 'L2 缓存', en: 'L2', cycles: 200, cap: '50 MB', tx: 492, ty: 150, tone: 'violet' },
  { name: 'HBM3 显存', en: 'HBM', cycles: 500, cap: '80 GB', tx: 648, ty: 150, tone: 'amber' },
]
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        <div className="min-w-0 flex-1">
          <div className="overflow-x-auto">
            <svg viewBox="0 0 720 260" className="w-full min-w-[540px] select-none" role="img" aria-label="存储层级取数路径图">
              {/* SM 框 */}
              <rect x={24} y={56} width={212} height={186} rx={6} className="fill-panel stroke-line2" />
              <text x={34} y={74} fontSize={11} className="fill-ink font-mono">SM（片上）</text>
              {/* 执行单元 */}
              <rect x={48} y={84} width={164} height={40} rx={4} className="fill-volt/10 stroke-volt/50" />
              <text x={130} y={108} fontSize={10.5} textAnchor="middle" className="fill-volt font-mono">执行单元（核心）</text>
              {/* 寄存器（SM 内部目标） */}
              <rect
                x={48} y={188} width={164} height={40} rx={4}
                className={`fill-bg2 ${cur && cur.idx === 0 ? 'stroke-volt' : 'stroke-line2'}`}
              />
              <text x={130} y={205} fontSize={10.5} textAnchor="middle" className="fill-ink2 font-mono">寄存器堆</text>
              <text x={130} y={220} fontSize={9} textAnchor="middle" className="fill-ink3 font-mono">≈1 周期 · 256 KB/SM</text>

              {/* 片上/片外分界 */}
              <line x1={566} x2={566} y1={40} y2={250} className="stroke-line" strokeDasharray="4 4" />
              <text x={560} y={36} fontSize={9} textAnchor="end" className="fill-ink3 font-mono">片上 ←</text>
              <text x={572} y={36} fontSize={9} className="fill-ink3 font-mono">→ 片外</text>
              ...
              {LEVELS.slice(1).map((lv, i) => {
                ...
                return (
                  <g key={lv.en}>
                    ...
                    <text x={lv.tx} y={172} fontSize={9} textAnchor="middle" className="fill-ink2 font-mono">≈{lv.cycles} 周期</text>
                  </g>
                )
              })}
              ...
              {dot && cur && (
                <g>
                  <rect
                    ...
                    className={`${TONE_FILL[LEVELS[cur.idx].tone]} drop-shadow-[0_0_6px_rgba(89,216,234,0.7)]`}
                  />
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {LEVELS.map((lv, i) => (
              <Btn key={lv.en} variant={i === 0 ? 'solid' : 'ghost'} onClick={() => doFetch(i)}>
                <span className={TONE_TEXT[lv.tone]}>▶</span> 从{lv.name}取数
              </Btn>
            ))}
            {st.queue.length > 0 && (
              <span className="self-center font-mono text-[11px] text-ink3">队列中 +{st.queue.length}</span>
            )}
          </div>
        </div>
        ...
        <div className="w-full shrink-0 rounded-md border border-line bg-bg2/60 p-4 lg:w-64">
          <div className="flex items-end justify-between gap-3">
            <Stat label="累计周期" value={counterValue.toLocaleString('en-US')} unit="cyc" tone="volt" size="lg" />
            <Stat label="取数次数" value={st.fetches} tone="ink" size="sm" />
          </div>
          <div className="mt-1 font-mono text-[11px] text-ink2">
            平均 <span className="text-cyan">{(st.total / Math.max(1, st.fetches)).toFixed(1)}</span> 周期/次
          </div>

          <div className="microlabel mb-2 mt-4">各级累计耗时</div>
          <div className="space-y-2.5">
            {LEVELS.map((lv, i) => (
              <div key={lv.en}>
                <div className="mb-0.5 flex items-baseline justify-between font-mono text-[10.5px]">
                  <span className={TONE_TEXT[lv.tone]}>
                    {lv.name} <span className="text-ink3">×{st.counts[i]}</span>
                  </span>
