  ...
  return (
    <>
      <p>
        {t(
          <>
            Run an algorithm that faithfully replays every floating-point multiply-add, yet finishes 2–4×
            faster with memory dropping from quadratic to linear. It sounds like cheating. It is what
            FlashAttention does, and there is no approximation, no pruning, no precision loss anywhere in it:
            the output matches standard attention to the bit. What it optimizes is not the arithmetic but the{' '}
            <strong>movement</strong> of data. The roofline model from <ChapterLink n={6} /> made the point:
            when a kernel's arithmetic intensity sits on the memory-bound side of the wall, speed comes down
            to how many bytes you shuttle between HBM and the chip, not how much you compute. Naive attention
            is a data-movement disaster, and the culprit is one S×S intermediate matrix that nobody ever
            actually wanted to <em>see</em>.
          </>,
          <>
            让一个算法把每一次浮点乘加都原封不动地照做，却快上 2~4 倍、显存占用从平方级降到线性。
            听起来像作弊。FlashAttention 干的就是这件事，而且全程没有近似、没有剪枝、没有降低精度：
            输出和标准 attention 逐位一致。它优化的不是计算，而是<strong>搬运</strong>。
            <ChapterLink n={6} />的 roofline 模型早已点破：当一个 kernel 的算术强度（arithmetic
            intensity）落在内存墙左侧，决定速度的是你在 HBM 和芯片之间搬了多少字节，而不是你算了多少。
            naive attention 偏偏就是个搬运灾难，而元凶是那块从来没人真正想「看到」的 S×S 中间矩阵。
          </>,
        )}
      </p>
      ...
      >
        <p>
          {t(
            <>
              Start with the formula from <ChapterLink n={7} />. Single-head attention computes:
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              once more. S and P have shape [S, S] (S is the sequence length): independent of head dim,
              growing <strong>quadratically</strong> with sequence length, and every head keeps its own copy.
            </>,
            <>
              ...
              <MathTex tex="O = PV" />，又把 P 完整读一遍。S 和 P 的形状是 [S, S]（S
        ...
        <p>
          {t(
            <>
              Put real numbers on it. Take S=8192, 32 heads, fp16: one head's score matrix is
            ...
            <>
              代入真实数字看看。取 S=8192、32 个头、fp16：一个头的 score 矩阵是 8192×8192×2B =
              128MiB，32 个头就是 <strong>4GiB</strong>，而这还只是「存一遍」的体积。
              上面的三步流程要让它在 HBM 上往返三次：
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              16GiB of HBM traffic, at H100's 3.35TB/s, takes about 5ms. The compute for this one attention
              layer? The two matmuls total roughly <MathTex tex="4S^2 d h \approx 1.1" /> TFLOPs, which 989
              TFLOPS of Tensor Core chews through in about 1.1ms. So you spend 1ms of work and wrap it in 5ms
              of data movement. Textbook memory-bound. The softmax in the middle is worse: it does one or two
              exponentials and adds per byte, an arithmetic intensity of about 1 FLOP/Byte, pinned to the
              far-left floor of the roofline. As long as S keeps making a round-trip through HBM, the bill is
              unavoidable, and it gets quadratically worse with sequence length.
            </>,
            <>
              16GiB 的 HBM 流量，在 H100 的 3.35TB/s 带宽下要走约 5ms。这一层 attention
              的总计算量呢？两个 matmul 合计约 <MathTex tex="4S^2 d h \approx 1.1" /> TFLOPs，989
              TFLOPS 的 Tensor Core 大约 1.1ms 就能算完。算 1ms 的活，外面裹了 5ms 的搬运，
              典型的 memory-bound。中间那步 softmax 更难看：它对每个字节只做一两次指数和加法，
              算术强度约 1 FLOP/Byte，在 roofline 图上贴着最左边的地板。只要 S
        ...
        <p>
          {t(
            <>
              ...
              all?</strong> The two matmuls are easy to please. <ChapterLink n={5} /> showed how tiling lets
              a matmul saturate the hardware. The real holdout is the softmax wedged in between. It
              normalizes over an <strong>entire row</strong>, so it seems to need a complete row of scores in
              hand before it can move at all. Untying that knot is the next section's job.
            </>,
            <>
              看清这一点，优化目标就明确了：<strong>能不能根本不把 S 写出来？</strong>
              两个 matmul 好伺候，<ChapterLink n={5} />讲过分块（tiling）可以让 matmul 吃满算力。
              真正的钉子户是夹在中间的 softmax：它要对<strong>整行</strong>做归一化，
              看起来非得先拿到完整的一行 score 才动得了手。解开这个结，是下一节的事。
            </>,
          )}
        </p>
      </Section>
      ...
      <Section
        ...
        lead={t(
          'Safe softmax needs the whole row’s max and sum, which looks un-tileable until you let yourself "fix the books" after the fact.',
      ...
      >
        <p>
          {t(
            <>
              Why softmax has to be "safe." Computing <MathTex tex="e^{x_i}/\sum_j e^{x_j}" /> straight
              ...
              practice everyone subtracts the row max first:
            </>,
            <>
              为什么 softmax 必须「安全」（safe softmax）。直接按定义算{' '}
              <MathTex tex="e^{x_i}/\sum_j e^{x_j}" /> 在浮点数里会爆：fp16 的最大值约
              65504，<MathTex tex="e^{x}" /> 在 x 超过 11 左右就溢出（fp32 也只撑到 x≈88）。
              所以实践中一律先减去整行最大值：
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              <MathTex tex="e^{-m}" />), but it keeps the exponent's argument from ever going positive, so it
              never overflows. The price is that you now need <strong>two passes</strong>: the first to find
              the row max m, the second to compute <MathTex tex="e^{x_i-m}" /> and accumulate the denominator
              l. That is exactly what tiling hates. K/V blocks stream in one at a time, and while you are
              processing the first block you have no idea whether a larger score lurks ahead.
            </>,
            <>
              减 m 不改变结果（分子分母同乘 <MathTex tex="e^{-m}" />），但能让指数的参数永不为正，
              也就永不溢出。代价是你现在要扫<strong>两遍</strong>：第一遍找整行的
              m，第二遍才能算 <MathTex tex="e^{x_i-m}" /> 并累加分母 l。这恰恰是分块最讨厌的事。
              K/V 块一块块流过来，处理第一块时你根本不知道后面会不会冒出更大的 score。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              The fix is online softmax, from a short 2018 note by NVIDIA's Milakov and Gimelshein. Turn
              "the row's (m, l)" into <strong>running state you maintain incrementally</strong>. Say you have
              already processed some blocks and hold the max m so far and the running denominator l; a new
              block now arrives, with local max <MathTex tex="m_b" /> and local sum{' '}
              <MathTex tex="l_b" />. The merge rule is:
            </>,
            <>
              解法是 online softmax，出自 NVIDIA 的 Milakov 和 Gimelshein 在 2018 年的一篇短文。
              把「整行的 (m, l)」变成<strong>可以增量维护的运行状态</strong>。
              假设你已经处理了一些块，手上握着目前为止的最大值 m 和分母累计 l；现在新来一块，
        ...
        <p>
          {t(
            <>
              ...
              "based at m." If the new block brings a larger max, the old basis is now too low. The remedy is
              to multiply the whole l by a <strong>rescale factor</strong>{' '}
              <MathTex tex="\alpha = e^{\,m-m_{\mathrm{new}}}" />: since{' '}
              <MathTex tex="e^{x_i-m}\cdot e^{\,m-m_{\mathrm{new}}}=e^{x_i-m_{\mathrm{new}}}" />, a single
              multiply shifts every historical term onto the new basis at once. It is an exact identity, not
              an approximation, so floating-point error stays at the last-bit (ulp) level.
            </>,
            <>
              为什么这是对的？此前的 l 是 <MathTex tex="\sum e^{x_i-m}" />，每一项都「以 m 为基准」。
              新块带来了更大的最大值，旧基准就偏低了。补救只需给整个 l 乘一个
              <strong>修正因子</strong> <MathTex tex="\alpha = e^{\,m-m_{\mathrm{new}}}" />：因为{' '}
              <MathTex tex="e^{x_i-m}\cdot e^{\,m-m_{\mathrm{new}}}=e^{x_i-m_{\mathrm{new}}}" />，
              一次乘法就把所有历史项整体换到了新基准上。这是恒等变形，不是近似，
        ...
        <p>
          {t(
            <>
              ...
              the same trick once more: keep an <strong>un-normalized</strong> output accumulator{' '}
              <MathTex tex="O=\sum_j e^{x_j-m}\,v_j" />. When a new block arrives and the max is refreshed,
              the old O gets multiplied by the same <MathTex tex="\alpha" /> to change basis, then the new
        ...
        <p>
          {t(
            <>
              Normalization (dividing by l) waits until all blocks are processed: one division, done. All the
              global information a full-row softmax needs is compressed into three running quantities,{' '}
              <strong>m, l, and the O accumulator</strong>. Their size depends only on the block's row count
              and head dim, and is <strong>independent of sequence length</strong>, so they fit comfortably in
              on-chip storage.
            </>,
            <>
              归一化（除以 l）推迟到所有块处理完，一次除法搞定。整行 softmax
              所需的全部全局信息，就压缩成了三个运行量：<strong>m、l、O 累计</strong>。
        ...
        <Callout type="insight" title={t('FlashAttention’s mathematical core is just two moves', 'FlashAttention 的数学核心只有两招')}>
          <p>
            {t(
              <>
                <strong>Defer normalization</strong>: accumulate the un-normalized numerators (l and O) first
                and do the single division last, so the intermediate steps can be tiled and accumulated
                freely. <strong>Rescale factor</strong>: when the basis (max) changes, multiply every
                historical accumulator by <MathTex tex="e^{\,m-m_{\mathrm{new}}}" /> to retroactively fix the
                books. Together they turn softmax from something that "must see the whole row" into a{' '}
                <strong>streaming reduction that obeys associativity</strong>. Every step is an exact
                identity, which is why FlashAttention is an exact algorithm rather than an approximation.
              </>,
              <>
                <strong>推迟归一化</strong>：先攒未归一化的分子（l 和 O），最后才做那一次除法，
                中间过程于是可以随便分块累加。<strong>修正因子</strong>：基准（max）变了不要紧，
                给所有历史累计量乘一个 <MathTex tex="e^{\,m-m_{\mathrm{new}}}" /> 就能追溯修账。
                两招合起来，softmax 从「必须看完整行」变成了「结合律成立的流式归约」。
                每一步都是精确的恒等变形，FlashAttention 因此是精确算法，不是近似。
              </>,
            )}
          </p>
        </Callout>
        ...
        <p>
          {t(
            <>
              ...
              online single-pass: the cursor reaches element i and updates (m, l) on the spot, with each
              update formula's numeric substitution laid out below the panel. Watch the key moment on the
              right. When the cursor hits a <strong>new max</strong>, the already-processed prefix cells flash
              amber: that is the instant their accumulators get multiplied by the rescale factor α and shifted
              to the new basis, and the "current estimate" below each cell shrinks at the same time.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              the scan, and you will see a cascade of large α corrections; however adversarial the order, the
              result is unchanged. Hit "Randomize" to swap the data (fixed seed, reproducible), and the final
              softmax in the bottom two rows always agrees digit-for-digit, with max error stuck at the 1e-16
              level or simply zero. Note the step count too: two-pass takes 16 steps to finish (two passes
              over the data = two HBM reads), online wraps up in 8. For a memory-bound kernel, reading once
              less is half the time.
            </>,
            <>
              ...
              0。再注意一下步数：两遍法走完要 16 步（数据过两遍 = HBM 读两遍），online 8 步收工。
              对一个 memory-bound 的 kernel 来说，少读一遍就是省下一半时间。
            </>,
          )}
        </p>
      </Section>
      ...
      >
        <p>
          {t(
            <>
              ...
              is "on the island," an order of magnitude faster but only 228KB per SM. All of FlashAttention's
              engineering goes into reorganizing attention so the S² data is only ever touched on the island.
              Split Q into Tr blocks (Br rows each), K/V into Tc blocks (Bc rows each), with block size set by
              the SRAM capacity M (on the order of M/4d). Then:
            </>,
            <>
              ...
              在「岛内」，快一个数量级但每个 SM 只有 228KB。FlashAttention 的全部工程，就是把
              attention 重组成只在岛内碰 S² 的数据：把 Q 切成 Tr 块（每块 Br 行）、K/V 切成 Tc
              块（每块 Bc 行），块的尺寸由 SRAM 容量 M 决定（量级是 M/4d）。然后：
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              ...
              and the 16GiB of quadratic traffic from the bill above simply vanishes.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              The cost? K/V gets re-read once per Q block row, so flash's HBM access volume is exactly{' '}
              <MathTex tex="\Theta(S^2 d^2 / M)" />, with the SRAM capacity M sitting in the denominator. As
              long as M is much larger than d² (which it is in reality: 228KB vs 128²×2B=32KB), it stays far
              below naive's <MathTex tex="\Theta(S^2)" />. In other words, <strong>the bigger the SRAM, the
              less K/V gets re-read</strong>, the same shape of trade-off we saw in <ChapterLink n={5} />'s
              matmul tiling.
            </>,
            <>
              代价呢？K/V 要被每个 Q 块行重读一遍，所以 flash 的 HBM 访问量精确说是{' '}
              <MathTex tex="\Theta(S^2 d^2 / M)" />，分母上正是 SRAM 容量
              M。只要 M 远大于 d²（现实如此：228KB vs 128²×2B=32KB），它就远小于 naive 的{' '}
              <MathTex tex="\Theta(S^2)" />。也就是说，<strong>SRAM 越大，K/V 重读得越少</strong>，
              这个权衡和<ChapterLink n={5} /> matmul tiling 里见过的形状一模一样。
            </>,
          )}
        </p>
      </Section>
      ...
      >
        <p>
          {t(
            <>
              The animation below runs the previous section's pseudocode frame by frame. The central grid is
              the "phantom" of the score matrix: dashed outlines mean these blocks never materialize to HBM,
              and the volt-highlighted one is the small block being computed in SRAM right now. The right
              panel simulates the on-chip state: the currently resident Q/K/V blocks, the merge progress of
              (m, l), and the O accumulator bar, which flashes each time a block is merged. The two cumulative
              bars at the bottom are the chapter's conclusion itself: naive's bill climbs as O(S²), flash's
              crawls as O(S·d), and by the end the gap opens past 14×.
            </>,
            <>
              下面的动画把上一节的伪代码逐帧跑一遍。中央网格是 score 矩阵的「影子」：
              虚线表示这些块从未物化到 HBM，volt 高亮的是此刻正在 SRAM
              里计算的小块。右侧面板模拟片上状态：当前驻留的 Q/K/V 块、(m, l)
              的合并进度、以及 O 累计条，每合并一块它就闪动一次。底部的双累计条就是本章的结论：
        ...
        <p>
          {t(
            <>
              ...
              wholesale</strong>. That is a gift of the tiled structure. A naive implementation typically
              computes the whole S, then fills the upper triangle with −∞, computing and shuttling it for
              nothing; flash never launches those computations and reads at the block level, so traffic and
              time drop by nearly half again. It is also why production FlashAttention kernels support
              "structured sparsity" masks like causal and sliding-window so naturally: the skip decision
              happens inside the block loop, at almost zero cost.
            </>,
            <>
              打开「causal mask」开关再跑一遍。decoder 的自回归 attention 里，第 i 个 query
              只能看见前 i 个 key，于是上三角的块<strong>整块跳过</strong>。这是分块结构送的礼物。
              naive 实现通常把整个 S 算完再把上三角填 −∞，白算白搬；flash
              在块粒度上根本不发起这些计算和读取，流量和时间再省接近一半。这也是为什么生产里的
      ...
      >
        <p>
          {t(
            <>
              ...
              one logsumexp per row (<MathTex tex="L=m+\log l" />, shape [S]). The backward pass needs S, so
              how does it cope? <strong>Recomputation</strong>: take Q, K and the saved L and recompute S<sub>ij</sub>{' '}
              block by block on the spot. FLOPs go up, yet it runs faster, because the HBM reads and writes it
              saves are worth far more than the extra compute. This is the worldview the course keeps
              returning to: on modern GPUs, <strong>compute is cheap, movement is expensive</strong>, and
              trading compute for movement is almost always a win. The payoff is concrete. At S=8192, one
              attention layer's activations drop from GiB-scale to MiB-scale, which is what turns long-context
              training from "impossible" into "routine."
            </>,
            <>
              <strong>显存：O(S²) → O(S)。</strong>前向不再存 S 和 P，只额外留下每行一个
              logsumexp（<MathTex tex="L=m+\log l" />，形状 [S]）。反向传播需要 S，怎么办？
              <strong>重计算（recomputation）</strong>：拿着 Q、K 和存好的 L
              按块把 S<sub>ij</sub> 现场重算一遍。FLOPs 变多了，速度反而更快，
              因为省下的 HBM 读写比多花的计算值钱得多。这是本课反复出现的世界观：
              在现代 GPU 上，<strong>计算便宜，搬运贵</strong>，用算换搬几乎总是赚的。
              收益很具体：S=8192 时单层 attention 激活从 GiB 级降到 MiB 级，长上下文训练就此从「不可能」变成「常规操作」。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              ...
              resident on-chip the whole time. It also cuts non-matmul FLOPs by minimizing the number of
              rescales (non-matmul ops can't use the Tensor Cores and cost about 16× more per FLOP), and
              extends parallelism from "batch × heads" to the sequence dimension, switching the inter-warp
              partition from slicing K to slicing Q to drop inter-warp synchronization and shared-memory
              shuffling. The result is about 70% of theoretical utilization on A100, roughly 2× faster than
              FA1.
            </>,
            <>
              <strong>FlashAttention-2（2023）：打掉调度浪费。</strong>FA1 的循环顺序其实是 K/V
              在外、Q 在内，逼着 (m, l, O) 反复进出 HBM。FA2 调换成本章伪代码的顺序（Q 外、K/V
              内），每个 Q 块的状态全程驻留片上。它还减少非 matmul FLOPs，
              把 rescale 的次数压到最少（非 matmul 运算走不了 Tensor Core，单位 FLOP
              贵约 16 倍）；并把并行度从「batch×头数」扩展到序列维度，warp 间分工从切 K
              改成切 Q，从而省掉 warp 间同步与共享内存中转。结果是 A100
        ...
        <p>
          {t(
            <>
              ...
              hardware sat unused by FA2: TMA (Tensor Memory Accelerator) can move data asynchronously, and
              WGMMA is the new generation of asynchronous matrix instructions. FA3 uses producer-consumer warp
              specialization plus "pingpong" scheduling to overlap one warp group's softmax with another warp
        ...
        <p>
          {t(
            <>
              To close in the language of the roofline: naive attention is pinned to the memory-bound side of
              the wall by its O(S²) traffic, with the softmax stage's arithmetic intensity of about 1
              FLOP/Byte sitting two orders of magnitude below H100's ridge point of about 295 FLOPs/Byte. Once
              FlashAttention strips the S² term out of the denominator (HBM byte count), the whole kernel's
              effective arithmetic intensity climbs back to the M/d scale (hundreds of FLOPs/Byte) and steps
              over the ridge into the compute-bound region. From there, attention's speed is dictated by the
              Tensor Cores, not the memory bus. That is the whole secret of "not one multiply removed, yet
              2–4× faster."
            </>,
            <>
              用 roofline 的语言收个尾：naive attention 被 O(S²) 的流量按在内存墙左侧，
              softmax 阶段的算术强度约 1 FLOP/Byte，离 H100 约 295 FLOPs/Byte
              的脊点差两个数量级。FlashAttention 把分母（HBM 字节数）里的 S²
              项砍掉之后，整个 kernel 的有效算术强度回升到 M/d 量级（数百 FLOPs/Byte），
      ...
      <Quiz
        ...
        options={[
          ...
          },
          {
            ...
            explain: t(
              'Kernel fusion does happen, but launch overhead is microsecond-scale and cannot explain a millisecond-scale gap. The real value of fusion is that the intermediate results never land in HBM, so what gets reduced is still the byte count of reads/writes.',
