...
const CARDS = [
  { gb: 24, label: '24G（4090）' },
  { gb: 80, label: '80G（A100/H100）' },
]
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-end gap-x-8 gap-y-3">
        <Stat label="权重大小" value={weightGB % 1 === 0 ? weightGB : weightGB.toFixed(1)} unit="GB" tone="amber" size="lg" />
        {CARDS.map((c) => {
        ...
        <Stat label="相对 FP16 理论 decode 加速" value={`${speedup % 1 === 0 ? speedup : speedup.toFixed(1)}×`} tone="volt" />
        <Stat label="预估 decode 速度" value={`~${tokS >= 100 ? Math.round(tokS) : tokS.toFixed(1)}`} unit="tok/s" tone="cyan" />
      </div>
    </Widget>
  )
}
