  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between">
        <span className="microlabel">8 个请求 · 一个静态批次</span>
        <Btn size="sm" variant="ghost" onClick={() => setSeed((s) => s + 1)}>⟳ 换一批</Btn>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="静态批处理时间线：最长请求拖住全批">
        <defs>
        ...
        <g transform={`translate(${labelW}, ${14 + N * rowH + 6})`} className="text-ink3">
          ...
          <text x={140} y={9} className="fill-current font-mono" fontSize={8}>完成但占着槽位（空转）</text>
        </g>
      </svg>
    </Figure>
  )
