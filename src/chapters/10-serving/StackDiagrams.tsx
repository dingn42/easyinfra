...
export function ChunkedPrefillSVG() {
  ...
  return (
    <Figure caption="chunked prefill：把 2000-token 的 prefill 切成若干 chunk，与其他请求的 decode token 拼进同一个迭代 —— 长 prompt 不再让邻居的 TPOT 卡顿。">
      <svg viewBox="0 0 640 132" className="w-full" role="img" aria-label="chunked prefill 对比示意">
        <text x={0} y={26} className="fill-current font-mono text-ink3" fontSize={9}>不切片</text>
        ...
        {row(14, [
          ...
          { c: 'fill-none' },
          { c: 'fill-none' },
          { c: 'fill-rose/30', t: '停摆', tc: 'text-rose' },
          { c: 'fill-rose/30', t: '停摆', tc: 'text-rose' },
          { c: 'fill-volt/80', t: 'decode' },
        ])}
        <text x={352} y={26} className="fill-current font-mono text-rose" fontSize={8}>← 邻居 TPOT 尖刺</text>

        <text x={0} y={76} className="fill-current font-mono text-ink3" fontSize={9}>切片混批</text>
        {row(64, [
        ...
        <text x={352} y={76} className="fill-current font-mono text-volt" fontSize={8}>← 平滑交错</text>
        <text x={120} y={108} className="fill-current font-mono text-ink3" fontSize={8}>→ 每格 = 引擎的一个迭代（iteration），cyan = prefill chunk，volt = 其他请求的 decode token</text>
      </svg>
    </Figure>
  )
}
...
export function PreemptionSVG() {
  return (
    <Figure caption="抢占（preemption）：KV cache 把显存挤爆时，调度器把某个请求请出去 —— 要么整体换出（swap）到主机内存，要么直接丢弃 KV、回头重算（recompute）。">
      <svg viewBox="0 0 640 120" className="w-full" role="img" aria-label="抢占与换出示意">
        {/* GPU 显存框 */}
        <rect x={10} y={14} width={300} height={88} rx={6} className="fill-bg2 stroke-line2" strokeWidth={1} />
        <text x={20} y={32} className="fill-current font-mono text-ink3" fontSize={9}>GPU 显存 · KV CACHE</text>
        {[0, 1, 2].map((i) => (
          <g key={i}>
            ...
            <text x={64 + i * 92} y={56.5} textAnchor="middle" className={`fill-current font-mono ${i === 2 ? 'text-rose' : 'text-amber'}`} fontSize={8}>
              {i === 2 ? 'REQ C ← 被抢占' : `REQ ${'AB'[i]} 的 KV`}
            </text>
          </g>
        ))}
        <rect x={22} y={72} width={130} height={22} rx={3} className="fill-volt/20 stroke-volt/60" strokeWidth={1} strokeDasharray="4 3" />
        <text x={87} y={86.5} textAnchor="middle" className="fill-current font-mono text-volt" fontSize={8}>腾出的空间 → 新请求</text>
        {/* 两条出路 */}
        ...
        <text x={527} y={46} textAnchor="middle" className="fill-current font-mono text-amber" fontSize={9}>SWAP：KV 换出到主机内存</text>
        <rect x={424} y={74} width={206} height={28} rx={4} className="fill-panel2 stroke-line2" strokeWidth={1} />
        <text x={527} y={92} textAnchor="middle" className="fill-current font-mono text-rose" fontSize={9}>RECOMPUTE：丢弃，回头重算</text>
      </svg>
    </Figure>
  )
}
...
export function SpeculativeSVG() {
  ...
  return (
    <Figure caption="speculative decoding：草稿模型串行起草 k 个 token（便宜），目标大模型一次前向并行验证。前缀命中的全收下，第一个不一致处之后作废 —— 一次「大模型时间」换回多个 token。">
      <svg viewBox="0 0 640 150" className="w-full" role="img" aria-label="speculative decoding 流程示意">
        {/* 草稿模型 */}
        ...
        <text x={70} y={48} textAnchor="middle" className="fill-current font-mono text-ink3" fontSize={7.5}>串行猜 k=4 个</text>
        ...
        <text x={408} y={48} textAnchor="middle" className="fill-current font-mono text-ink3" fontSize={7.5}>一次前向，4 个并行验证</text>
        {/* 验证结果 */}
        ...
        <text x={160} y={134} className="fill-current font-mono text-ink3" fontSize={8}>
          命中 3 个 + 大模型自己给出的修正 token：一步净赚 4 个 token
        </text>
        <text x={494} y={37} className="fill-current font-mono text-ink3" fontSize={8}>← 把串行 decode</text>
        <text x={494} y={49} className="fill-current font-mono text-ink3" fontSize={8}>变成并行验证</text>
      </svg>
    </Figure>
  )
}
...
export function ServingStackSVG() {
  const layers = [
    { name: 'ROUTER / 网关', desc: '多副本负载均衡 · prefix 亲和路由 · 限流', tone: 'text-ink2', stroke: 'stroke-line2' },
    { name: 'SCHEDULER / 调度器', desc: '连续批处理 · chunked prefill · 优先级与抢占', tone: 'text-volt', stroke: 'stroke-volt/60' },
    { name: 'ENGINE / 执行引擎', desc: 'KV cache 分页管理 · prefix 复用 · speculative', tone: 'text-cyan', stroke: 'stroke-cyan/60' },
    { name: 'KERNELS / 算子层', desc: 'FlashAttention · Paged KV · 融合 GEMM', tone: 'text-amber', stroke: 'stroke-amber/60' },
  ]
  return (
    <Figure caption="一个推理服务栈的分层全景：请求自上而下，每一层解决一类问题。本章的主角是中间两层 —— 调度器与引擎。">
      <svg viewBox="0 0 640 248" className="w-full" role="img" aria-label="推理服务栈分层示意">
        {layers.map((l, i) => (
          <g key={l.name}>
        ...
        <text x={62} y={30} textAnchor="end" className="fill-current font-mono text-ink3" fontSize={8}>请求 ↓</text>
