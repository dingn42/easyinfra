    ...
    <>
      <p>
        {t(
          <>
            ...
            it starts spelling out tokens one by one. But in the datacenter, that H100 is probably juggling
            dozens of sessions at this instant — your question, someone else&apos;s code completion, an
            agent&apos;s 17th tool call, all crammed onto the same chip taking turns. The illusion that every
            user owns the model outright is bought not with more cards but with <strong>scheduling</strong>.
            This chapter cracks open the inference engine to see what systems like vLLM and TensorRT-LLM are
            actually scheduling — and why a good scheduler can lift the throughput of a single card by an order
            of magnitude.
          </>,
          <>
      ...
      >
        <p>
          {t(
            <>
              ...
              : the hundreds or thousands of tokens in the prompt are <em>already known</em>, so they can be
              packed into one big matrix and computed in a single forward pass. This is a glorious large matmul
              — arithmetic intensity climbs linearly with prompt length, the GPU&apos;s tensor cores are kept
              well fed, and effective compute utilization can reach 60%–70%.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              below 1%. The same card flips from a fully loaded blast furnace into an almost-idle conveyor belt.
            </>,
            <>
              第二段叫{' '}
              <Term t={t('decode', 'decode（解码）')}>
        ...
        <p>
          {t(
            <>
              These two lives map onto the two latency metrics users feel most acutely:{' '}
              <strong>TTFT</strong> (Time To First Token) is almost entirely set by prefill — the longer the
              prompt and the longer the queue, the longer you stare at a blinking cursor; <strong>TPOT</strong>{' '}
              (Time Per Output Token) is set by decode — it governs how fast the words spill out. The
              experiment below puts the two lives side by side; watch where the two points land on the roofline
              to the right: prefill pressing against the compute ceiling, decode mired on the bandwidth slope.
            </>,
            <>
      ...
      >
        <p>
          {t(
            <>
              ...
              produces 32 tokens — arithmetic intensity multiplies by 32 outright. As long as you haven&apos;t
              hit the compute ceiling, throughput rises <em>linearly</em> with batch size while each user&apos;s
              perceived TPOT barely moves: the time goes into reading weights anyway, so computing 31 extra
              tokens comes almost for free. This is that rare &quot;nearly free lunch&quot; in serving.
            </>,
            <>
        ...
        <Callout type="insight" title={t('The first principle of inference systems', '推理系统的第一性原理')}>
          <p>
            {t(
              <>
                Trace every fancy scheduling trick in this chapter back to its root and you reach the same
                sentence: <strong>every decode step pays a fixed cost to &quot;read all the weights,&quot; and
                the scheduler&apos;s mission is to make each payment serve as many tokens as possible</strong>.
                Batching amortizes it along the &quot;number of users&quot; axis; speculative decoding amortizes
                it along the &quot;time&quot; axis (one forward pass verifies several draft tokens); and
                continuous batching guarantees this amortization never lapses for an instant. Grasp this and
                vLLM&apos;s source code reduces to mere engineering detail.
              </>,
              <>
        ...
        <p>
          {t(
            <>
              ...
              admit the next batch once the entire current one is done — this is <strong>static
              batching</strong>, and it&apos;s how many early in-house serving stacks were really written. Its
              flaw hides in one unremarkable fact: <em>nobody knows how long a request will generate</em>. One
              user asks &quot;translate this word&quot; and is done in 16 tokens; another wants &quot;write a
              two-thousand-word summary&quot; and runs for hundreds of steps. Entering and leaving the batch
              together means everyone is dragged along until the longest one finishes.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              hangs there idling (the gray hatching in the figure) — the GPU keeps paying the weight-read cost
              for &quot;slots with no live work,&quot; and the effective batch shrinks as it runs. Worse is the
              hard boundary between batches: even if a new request arrives one second after the batch starts, it
              must wait for the entire batch to finish before it can board, so average wait time blows up. The
              larger the variance in output length, the more vicious the bucket effect — this isn&apos;t a
              matter of a sloppy implementation, it&apos;s that the &quot;batch&quot; granularity itself is
              wrong.
            </>,
            <>
      ...
      >
        <p>
          {t(
            <>
              The 2022 Orca paper poked through this window: since decode already iterates one step at a time,
              why must scheduling operate on the granularity of a &quot;whole batch&quot;? Move the decision
              point to <strong>between each iteration</strong>: after every step, whoever has finished leaves
              immediately and returns its slot; if the queue holds a new request, it boards immediately — first
              its prefill, then it joins the decode crowd. This is <strong>continuous batching (also called
              in-flight batching / iteration-level scheduling)</strong> — the scheduling core of today&apos;s
              vLLM, TensorRT-LLM, and SGLang without exception.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              The gain isn&apos;t a few-percent tweak. There are no more passengers dragged along inside the
              batch, so the GPU&apos;s effective batch stays glued to the concurrency limit; a new request&apos;s
              wait shrinks from &quot;until the whole batch ends&quot; to &quot;until the next iteration.&quot;
              On real workloads with high output-length variance, throughput gains of 2–5x are common, and the
              improvement in tail latency (P95/P99) is even more dramatic. The simulator below replays the same
              fixed-seed request stream under both schedulers: run STATIC first and watch the sawtooth
              collapses in the utilization bar; switch to CONTINUOUS and watch new requests fill empty slots on
              the fly, pinning utilization high. Then try cranking up arrival rate λ and widening the output
              range — the gap gets uglier.
            </>,
            <>
        ...
        <Quiz
          ...
          options={[
            ...
            },
            {
              ...
              correct: true,
              explain: t(
                'Right. The key insight is that decode already beats time in iterations, so the scheduler has no reason to decide at a coarser granularity. Release-on-finish plus admit-on-arrival makes the bucket effect vanish.',
            ...
            },
            {
              ...
              explain: t(
                'Length-predicting schedulers do exist as research, but the beauty of continuous batching is precisely that it needs no prediction — whoever finishes first leaves first, so the schedule is inherently adaptive.',
      ...
      >
        <p>
          {t(
            <>
              ...
              iteration long — users already in decode suddenly feel the words pause, a spike in TPOT. The fix
              is to slice the long prompt into chunks of a few hundred tokens and mix only one small chunk into
              each iteration, pairing it with decode tokens into a modestly sized hybrid batch. The cost is
              slightly longer TTFT, traded for everyone&apos;s TPOT staying smooth — a textbook case of
              &quot;trading peak for variance.&quot;
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              bring it back when there&apos;s room, or simply discard it and recompute via one prefill when it
              re-queues. Swapping costs PCIe bandwidth, recompute costs compute, and vLLM weighs the choice by
              KV size. To the user this is just a latency wobble — far better than an OOM that crashes the whole
              engine.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              attacks &quot;serial&quot; itself. Take a much smaller draft model to serially guess k tokens — it
              is small, so it guesses fast; then let the big model verify those k positions in <em>one</em>{' '}
              parallel forward pass, in a way that is provably identical in distribution to the big model
              sampling them one at a time. Hit 3 of them and you&apos;ve produced 4 tokens (including the
              correction) in one &quot;big-model time&quot;. In essence it converts serial decode into parallel
              verification — running decode&apos;s work with prefill&apos;s posture. The more accurate the
              draft, the bigger the win; the cost is keeping an extra small model around (or, like EAGLE/Medusa,
              growing a draft head off the big model itself).
            </>,
            <>
        ...
        <p>
          {t(
            <>
              Assembling these parts into a whole machine: a production-grade serving stack roughly splits into
      ...
      >
        <p>
          {t(
            'The operational language of serving is built from four metrics, each standing on a different vantage:',
            '推理服务的运营语言由四个指标构成，每个都站在不同的立场上：',
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              notch — there&apos;s no &quot;throughput-and-latency buy-one-get-one&quot; deal. So tuning a
              production system is never &quot;bigger is better&quot; but a constrained optimization problem:
              first agree on the SLO with the business (a service level objective like P95 TTFT &lt; 2 s, TPOT
              &lt; 50 ms), then turn the knobs — concurrency limit, chunked-prefill chunk size, preemption
              threshold — to wherever <em>goodput</em> is maximized. The same engine tuned for a chat product
              versus offline batch summarization lands on completely different parameters — the former trades
              throughput for latency, the latter the reverse. Once the metrics are defined, those few sliders in
              the Section 4 simulator are the scaled-down versions of the knobs you&apos;ll be turning in
              production.
            </>,
            <>
      ...
      >
        <ul>
          ...
          <li>
            {t(
              <>
                <strong>Continuous batching</strong> drops scheduling granularity to the iteration level —
                yield on finish, admit on arrival — and is the shared heart of every modern inference engine.
              </>,
              <>
