...
export function PrefillDecodeLab() {
  ...
  return (
    <Widget
      index={1}
      title="Prefill / Decode 对比"
      subtitle="同一个模型，两段完全不同的人生"
      onReset={reset}
      footer={
        <>
    ...
    >
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_232px]">
        ...
        <div className="min-w-0">
          <svg viewBox="0 0 560 252" className="w-full" role="img" aria-label="prefill 与 decode 两阶段的 token 流动对比">
            {/* ───────── PREFILL 泳道 ───────── */}
            <g className="text-ink3">
              <text x={4} y={laneY[0] - 4} className="fill-current font-mono" fontSize={9} letterSpacing={1}>
                PREFILL · 整段 PROMPT 并行
              </text>
            </g>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-md border border-line bg-bg2 p-3">
            <div className="microlabel mb-2">两阶段在 ROOFLINE 上的位置</div>
            <svg viewBox="0 0 200 130" className="w-full" role="img" aria-label="roofline 上 prefill 接近算力屋顶，decode 深陷带宽斜坡">
              {/* 轴 */}
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-3">
            <Stat label="PREFILL 利用率" value={Math.round(PREFILL_MFU * 100)} unit="%" tone="volt" />
            <Stat label="DECODE 利用率" value={(decodeMfu * 100).toFixed(1)} unit="%" tone="amber" />
