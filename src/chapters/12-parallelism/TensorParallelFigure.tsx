...
export function TensorParallelFigure() {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 720 320" className="w-full select-none" role="img" aria-label={t('Tensor parallelism column-split/row-split diagram', '张量并行列切/行切示意图')}>
        {/* ── GPU0 行 ── */}
        {[0, 1].map((g) => {
          const y0 = g === 0 ? 30 : 150
        ...
        <g transform="translate(10, 268)">
          ...
          <text x={0} y={38} fontSize={10.5} fontFamily={mono} fill="var(--color-ink2)">
            {t('InfiniBand (cross-box)', 'InfiniBand (跨机)')}
          </text>
          ...
          <text x={158} y={38} fontSize={10} fontFamily={mono} fill="var(--color-rose)">
            {t('~50GB/s (18× less)', '~50GB/s（差 18 倍）')}
          </text>
        </g>
      </svg>
    </Figure>
