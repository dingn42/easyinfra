...
export function ZeroBars() {
  ...
  return (
    <Figure caption={`以 7B 模型（混合精度训练，不含激活）为例：DP=${n} 张卡时，每卡的「三件套」显存。ZeRO 每升一档，又一类状态被切成 1/N。`}>
      <div className="mb-3 max-w-[280px]">
        <Slider label="DP 卡数 N" value={exp} min={1} max={6} onChange={setExp} fmt={(v) => String(2 ** v)} unit="卡" />
      </div>
      <svg viewBox={`0 0 ${VW} ${VH}`} className="w-full select-none" role="img" aria-label="ZeRO 各 stage 每卡显存柱状图">
        {/* y 轴刻度 */}
        {[0, 40, 80, 120].map((v) => (
          <g key={v}>
        ...
        <g transform={`translate(${x0}, ${VH - 8})`} fontSize={10} fontFamily="var(--font-mono, monospace)" fill="var(--color-ink3)">
          ...
          <text x={13}>权重 14G</text>
          <rect x={86} width={9} height={9} y={-8} fill="var(--color-amber)" opacity={0.8} />
          <text x={99}>梯度 14G</text>
          <rect x={172} width={9} height={9} y={-8} fill="var(--color-violet)" opacity={0.8} />
          <text x={185}>优化器态 84G</text>
        </g>
      </svg>
    </Figure>
  )
