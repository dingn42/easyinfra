...
const HOOK_CODE = `// kernel A：每个 warp 触发 4 个 32B 事务，带宽利用率 100%
out[i] = a[i] * 2.0f;

// kernel B：指令数一模一样，却是 32 个事务，利用率 12.5%
out[i] = a[i * 8] * 2.0f;`

const AOS_CODE = `// AoS（Array of Structures）：粒子打包成结构体，再排成数组
...
const SOA_CODE = `// SoA（Structure of Arrays）：每个字段一条独立数组
...
const TRANSPOSE_CODE = `__global__ void transpose(float* out, const float* in, int n) {
...
const DOUBLE_SYNC_CODE = `for (int t = 0; t < numTiles; ++t) {
...
const HIER_ROWS: [string, string, string, string, string][] = [
  ['寄存器', '片上 · 每线程', '~1 cycle', '线程私有', '一切计算的起点；每线程上限 255 个'],
  ['Local memory', '显存（没错，在板上）', '~400+ cycles', '线程私有', '寄存器溢出（spill）的去处，名字最骗人'],
  ['Shared memory', '片上 · 每 SM', '~20–30 cycles', 'block 内共享', '可编程缓存，本章主角'],
  ['Constant', '显存 + 专用缓存', '命中后近似寄存器', '全 grid 只读', '全 warp 读同一地址时一次广播'],
  ['L2 cache', '片上 · 全卡共享', '~200 cycles', '所有访问路过', '透明缓存；A100 40 MB / H100 50 MB'],
  ['Global (HBM)', '板上显存', '~400–600 cycles', '全 grid', '主战场；A100 约 1.9 TB/s / H100 3.35 TB/s'],
  ['Pinned host mem', '主机内存（页锁定）', '走 PCIe / NVLink', 'Host ↔ Device', 'cudaMemcpyAsync 重叠拷贝与计算的前提'],
]
  ...
  return (
    <>
      ...
      <Section
        index={1}
        title="全局内存按「事务」收费"
        lead="显存不做零售生意：你要 1 个字节，它也按 32 字节起步价给你搬。"
      >
        <p>
        ...
        <CodeBlock code={HOOK_CODE} lang="cuda" title="同样的指令数，10 倍的差距" />
        <p>
        ...
        <Callout type="insight" title="带宽是按事务计费的">
          <p>
      ...
      <Section
        index={2}
        title="实验：亲手摸一摸事务"
        lead="与其背规则，不如拖一拖滑杆，看 32 条连线怎么砸在内存条带上。"
      >
        <p>
        ...
        <Quiz
          ...
          options={[
            {
              text: '4 个——硬件能跳过没人要的字节',
              explain: '硬件不能跳着搬：32 B 段是最小传输粒度，段里哪怕只有一个字节被要也得整段搬。',
            },
            {
              text: '8 个',
              correct: true,
            ...
            {
              text: '16 个',
              explain: '每个 32 B 段装 8 个 float，stride=2 时每段里有 4 个被读到，共 256 B / 32 B = 8 段，不是 16。',
            },
            {
              text: '32 个——每个线程一个事务',
              explain: '那是 stride ≥ 8 时的封顶情况；stride=2 时多个线程还能共享同一个段。',
            },
          ]}
        />
      </Section>
      ...
      <Section
        index={3}
        title="AoS vs SoA：struct 是怎么谋杀带宽的"
        lead="没人会故意写 a[i*3]，但写 p[i].x 的人到处都是——这是同一件事。"
      >
        <p>
        ...
        <AosSoaLab />
        <p>对照着代码看会更清楚——kernel 逻辑一个字没变，变的只是数据在内存里的「站位」：</p>
        <CodeBlock code={AOS_CODE} lang="cuda" title="aos.cu —— 12 个事务 / warp" highlight={[6]} />
        <CodeBlock code={SOA_CODE} lang="cuda" title="soa.cu —— 4 个事务 / warp" highlight={[6]} />
        <p>
      ...
      <Section
        index={4}
        title="Shared memory：片上的可编程缓存"
        lead="改不了算法的访问模式？那就在片上找块地，自己当缓存管理员。"
      >
        <p>
        ...
        <CodeBlock
          code={TRANSPOSE_CODE}
          lang="cuda"
          title="transpose.cu —— shared memory 当中转站"
          highlight={[2, 7, 12]}
        />
        <p>
        ...
        <CodeBlock code={DOUBLE_SYNC_CODE} lang="cuda" title="为什么循环里要 sync 两次" highlight={[3, 6]} />
        <p>
          屏障 ① 防「写后读」竞争：没写完就被读。屏障 ② 防「读后写」竞争：跑得快的线程进入下一轮迭代，把慢
          线程还没读完的数据覆盖掉。少写第二道 sync 的 bug 极其阴险——多数时候结果碰巧正确，换张卡、换个
          block 大小就偶发出错。
        </p>
        <Callout type="warn" title="__syncthreads() 只管 block 内">
          <p>
      ...
      <Section
        index={5}
        title="Bank conflict：shared memory 自己的坑"
        lead="片上不收 32B 段的税，但它有自己的税种。"
      >
        <p>
        ...
        <Quiz
          ...
          options={[
            {
              text: '每行多 1 个 word，相邻行的同一列在 bank 视角下错开一格；列访问的 stride 变成与 32 互质的 33',
              correct: true,
              explain:
                '正解。按列访问时 lane i 的地址 stride 等于行宽（33 个 word），bank = 33i mod 32 = i，32 个访问均匀落到 32 个不同 bank，一拍完成。',
            },
            {
              text: '数组更大，L1 缓存命中率更高',
              explain: 'shared memory 本身就是片上 SRAM，不经过 L1；多出的一列与缓存无关。',
            },
            {
              text: '编译器看到非 2 的幂会自动重排数据布局',
              explain: '编译器不会偷偷改你声明的布局；padding 是程序员手动完成的、对编译器完全透明的技巧。',
            },
            {
              text: '多出的一列用来存 __syncthreads() 的标志位',
              explain: '屏障由硬件指令实现，不占 shared memory；那一列纯粹是被故意浪费掉的空间。',
            },
          ]}
        />
      </Section>
      ...
      <Section
        index={6}
        title="存储层级的其他角色"
        lead="主角讲完了，配角们各有一句话的戏份——但关键时刻都能要你的命。"
      >
        <p>
        ...
        <Figure caption="GPU 存储层级速查表：延迟为数量级估计，随架构浮动；容量与带宽以 A100 / H100 为参考">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse font-mono text-[12.5px]">
              <thead>
                <tr className="border-b border-line2 text-left">
                  <th className="microlabel py-2 pr-3 font-normal">存储</th>
                  <th className="microlabel py-2 pr-3 font-normal">位置</th>
                  <th className="microlabel py-2 pr-3 font-normal">延迟（约）</th>
                  <th className="microlabel py-2 pr-3 font-normal">作用域</th>
                  <th className="microlabel py-2 font-normal">一句话定位</th>
                </tr>
              </thead>
              <tbody className="text-text">
                {HIER_ROWS.map((r, i) => (
                  <tr key={i} className="border-b border-line last:border-0">
                    <td className="py-2 pr-3 whitespace-nowrap text-ink">{r[0]}</td>
                    <td className="py-2 pr-3 text-ink2">{r[1]}</td>
                    <td className="py-2 pr-3 tabular-nums text-amber">{r[2]}</td>
                    <td className="py-2 pr-3 text-ink2">{r[3]}</td>
                    <td className="py-2 text-ink2">{r[4]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Section index={7} title="总结与延伸阅读" lead="这一章只有一个中心思想：让搬来的每个字节都被用上。">
        <p>把本章的直觉压缩成五条，够你诊断大多数访存问题：</p>
        <p>
