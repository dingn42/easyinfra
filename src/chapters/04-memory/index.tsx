    ...
    <>
      <p>
        {t(
          <>
            ...
            on-chip shared memory launders a bad access pattern into a good one. The conclusion, stated up front:{' '}
            <strong>most kernels are bound by memory, not compute</strong>. Learn to read memory access and you've
            mastered most of GPU performance tuning.
          </>,
          <>
      ...
      >
        <p>
          {t(
            <>
              Global memory is the tens of gigabytes of HBM on the card. Its bandwidth looks terrifying — about
              1.9 TB/s on A100, 3.35 TB/s on H100 — but the number comes with a condition: you have to access it
              the way it likes. The HBM interface's minimum transfer granularity is not one byte but a 32 B{' '}
              <strong>sector</strong>. GPU hardware places a coalescer in front of the memory subsystem: when a{' '}
            ...
            <>
              ...
              约 3.35 TB/s——但这个数字有个前提：你得按它喜欢的方式访问。HBM 的接口一次传输的最小粒度不是
        ...
        <p>
          {t(
            <>
              Let's do the arithmetic. 32 threads each read a 4 B float, total demand 128 B. If thread{' '}
              <code>i</code> reads <code>a[i]</code> — contiguous addresses, aligned base — those 128 B tile
              exactly across 4 adjacent 32 B sectors, the hardware issues 4 transactions, and every byte fetched
              has a taker: 100% utilization. This ideal state, where a warp's accesses pack into as few sectors as
              possible, is called <strong>memory coalescing</strong>. It is not a compiler optimization and needs
              no special instruction from you — it's decided on the fly by the hardware purely from{' '}
              <em>the shape of those 32 addresses in a single cycle</em>.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              touching 8 sectors, 8 transactions hauling back 256 B — but you still only wanted 128 B, so
              utilization drops to 50%. Crank the stride further and at <code>a[i * 8]</code> each float owns its
              own 32 B sector: 32 transactions fetch 1024 B, only 128 B of which is useful — 12.5% utilization,
              rock bottom. Random access (gather) ends up similar: addresses scatter wherever they land, worst
              case again one transaction per thread. That's the entire secret behind the 10x gap from the opening:
              kernel B uses a 1.9 TB/s bus at 12.5% efficiency, so its <em>effective</em> bandwidth is barely two
              hundred-some GB/s.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              Two footnotes. First, alignment is part of the bill too: if a 128 B contiguous access doesn't start
              on a 32 B boundary, it straddles one extra sector and 4 transactions become 5 — which is why the
              pointer from <code>cudaMalloc</code> is naturally 256 B aligned, and why you should watch out when
              doing your own pointer offsets. Second, the L1/L2 caches do exist (a 128 B cache line is made of 4
              sectors), but for a big array streamed through once, the cache can't help — streaming workloads are
              entirely transaction-count bound.
            </>,
            <>
              两个补充细节。其一，对齐（alignment）也参与计费：如果 128 B 的连续访问首地址不在 32 B 边界上，
              会多骑跨一个段，4 个事务变 5 个——所以 <code>cudaMalloc</code> 返回的指针天然 256 B 对齐，自己做
              指针偏移时要留心。其二，L1/L2 缓存确实存在（cache line 128 B，由 4 个 sector 组成），但对一次性
          ...
          <p>
            {t(
              <>
                ...
                ships 32 B — the wasted part still occupies trucks and still burns power. So "optimize memory
                access," translated into hardware terms, comes down to one sentence:{' '}
                <strong>make every byte you move actually get used</strong>. Contiguous addresses within a warp is
                the simplest and most important way to get there.
              </>,
              <>
                把显存想成一个只做批发的仓库：最小发货单位 32 B，你要 4 B 它也发 32 B，浪费的部分照样占运力、
                照样耗电。所以「优化访存」这四个字翻译成硬件语言只有一句话——
                <strong>让每一个被搬运的字节都被用上</strong>。warp 内地址连续，是达成这件事最简单也最重要的手段。
              </>,
            )}
          </p>
        </Callout>
      ...
      >
        <p>
          {t(
            <>
              ...
              read request, and a touched sector lights up amber —{' '}
              <strong>count the lit cells and you have the transaction count</strong>. Switch to "stride" mode and
        ...
        <p>
          {t(
            <>
              Two moments worth pausing on. First, going from stride 1 to 2 instantly doubles the transaction
              count — the waste isn't gradual, it starts the instant you leave "contiguous." Second, past stride 8
              the transaction count pins at 32 and won't move: each float already owns a sector, so the penalty is
              capped (the cache layer keeps degrading, but transactions have bottomed out). Performance doesn't
              fall forever — it falls only to the floor of "one transaction per request."
            </>,
            <>
              有两个值得停下来体会的点。第一，stride 从 1 变到 2，事务数立刻翻倍——浪费不是渐进发生的，从你离开
              「连续」的那一刻就开始了。第二，stride 涨到 8 以后再涨，事务数钉死在 32 不动：每个 float 已经独占
      ...
      >
        <p>
          {t(
            <>
              Strided access sounds like a deliberately contrived bad example, but it has an extremely common
              disguise: <strong>the array of structs</strong>. Imagine a particle simulation where each particle
              is <code>{'struct Particle { float x, y, z; }'}</code> and a million particles sit in an array —
              this "array of structures" layout is called <strong>AoS (Array of Structures)</strong>. Now write a
              kernel that touches only the x coordinate: thread <code>i</code> reads <code>p[i].x</code>. Looks
              harmless, but work out the addresses: neighboring threads read addresses 12 B apart — a strided
              access with a stride of 3 floats, just dressed up in a struct.
            </>,
            <>
              ...
              kernel：线程 <code>i</code> 读 <code>p[i].x</code>。看起来人畜无害，但算一下地址：相邻线程读的地址差
              12 B——这就是一个 stride 为 3 个 float 的跨步访问，只是穿了件 struct 的外衣。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              ...
              transactions, 33% utilization. The y and z hauled back have no takers, but you pay for the bandwidth
              anyway. The cure is to rotate the layout: store x, y, z each in its own independent array. This is{' '}
              <strong>SoA (Structure of Arrays)</strong>. The same kernel now reads <code>p.x[i]</code>, the
              addresses are contiguous again, 4 transactions, 100% utilization, two thirds of the bandwidth saved
              outright. The lab below lets you flip between the two layouts — watch the memory strip rearrange and
              the 12 amber sectors collapse into 4:
            </>,
            <>
              一个 warp 要的 32 个 x 散布在 384 B（32 × 12 B）的范围上，触及 12 个 32 B 段——12 个事务，利用率
              33%。搬回来的 y 和 z 没人要，但带宽照付。解药是把布局转个方向：x、y、z 各自存成一条独立的数组，
        ...
        <p>
          {t(
            <>
              On the CPU this lesson is a nice-to-have (the cache covers for you); on the GPU it's life-or-death.
              A CPU scanning AoS sequentially on one thread can still reuse the cache line, whereas on a GPU 32
              threads each poke a different struct <em>in the same cycle</em>, and the waste is multiplied by 32.
              Worth noting in passing: the tensors in deep learning frameworks — one big contiguous array sliced
              by dimension — are essentially SoA thinking taken all the way. And if the kernel really does need
              x/y/z together, there's a third path: use a vector type like <code>float4</code> so one thread moves
              16 B in one instruction — contiguous and instruction-thrifty at once.
            </>,
            <>
              这条经验在 CPU 世界是「锦上添花」（缓存会兜底），在 GPU 世界是「生死攸关」：CPU 单线程顺序扫
              AoS 时缓存行还能复用，而 GPU 上 32 个线程<em>同一拍</em>各自戳一个 struct，浪费被乘以 32。顺带说一句，
              深度学习框架里的 tensor——一整块连续的大数组，按维度切分——本质上就是把 SoA 思想贯彻到底的产物。
      ...
      >
        <p>
          {t(
            <>
              ...
              write by column — one of the two is doomed to be strided. That's not bad code, it's the shape of the
              problem itself. This is where <strong>shared memory</strong> steps in. It's a slab of SRAM on each
              SM (up to 164 KB per SM on A100, 228 KB on H100), with ~20–30 cycle latency — more than an order of
              magnitude below global memory — and far higher aggregate bandwidth. Most important of all: it's{' '}
              <strong>under your programmatic control</strong>. A hardware cache can only guess what you want;
              shared memory does what you tell it.
            </>,
            <>
              ...
              数量级以上；聚合带宽更是高出一大截。最关键的是：它<strong>由你编程管理</strong>——硬件缓存只能猜你
        ...
        <p>
          {t(
            <>
              There are only two new faces in the syntax. One is the <code>__shared__</code> qualifier: it
              declares an array shared by all threads in a block, living as long as the block does. The other is{' '}
              <code>__syncthreads()</code>: a block-level barrier that blocks at the call site until <em>every</em>{' '}
              thread in the block has arrived. There's exactly one rule, and it's critical —{' '}
              <strong>all threads must reach the same barrier</strong>. Put it inside a branch only some threads
              take (e.g. inside <code>{'if (tid < 16)'}</code>) and they never all arrive: deadlock at best,
            ...
            <>
              ...
              内<em>所有</em>线程到齐才放行。规则只有一条但极其重要——<strong>所有线程都必须到达同一个屏障</strong>。
              把它放进只有部分线程会走的分支里（比如 <code>{'if (tid < 16)'}</code> 内部），到不齐，轻则死锁，
              重则未定义行为。
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              coalesced load to bring it on-chip, then every reuse afterward is billed at on-chip rates.
              <br />
              <strong>(2) A staging area to rewrite the access pattern</strong> — for problems like transpose
              where one of read/write must be strided, move the strided end into shared memory: both ends of
              global memory stay on the coalesced path, and the messy scrambling work happens on-chip, where
              there's no 32 B sector tax.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              Barrier (1) guards against a write-then-read race: reading before the write is done. Barrier (2)
              guards against a read-then-write race: a fast thread enters the next
              iteration and overwrites data a slow thread hasn't finished reading. The bug from omitting the
              second sync is insidious — the result is usually correct by luck, then fails intermittently the
              moment you swap GPUs or change the block size.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              sliced into <strong>32 banks</strong>, addressed round-robin by 4 B words — word 0 in bank 0, word 1
              in bank 1, ... word 32 wraps back to bank 0. Each bank can serve <em>one address</em> per cycle. So
              a warp's 32 accesses complete in a single cycle if they fall evenly across 32 distinct banks (or if
              several threads read the <em>same</em> address — the hardware just broadcasts). But if N threads
              land on <strong>different addresses in the same bank</strong>, that's an{' '}
        ...
        <p>
          {t(
            <>
              ...
              word <code>33i</code>, landing in bank <code>33i mod 32 = i</code> — the 32 lanes spread evenly
              across all 32 banks. This is the entire principle behind the <strong>padding trick</strong>: declare
              the tile <code>[32][33]</code>, leaving one never-used word at the end of each row, so consecutive
              rows are offset by one in bank-space, and the column-access stride goes from 32 to 33. The conflict
              vanishes. The cost is 128 B of on-chip space in exchange for up to a 32x speedup — the highest
              bang-for-buck one-line change in all of GPU programming.
            </>,
            <>
              stride=33 为什么神奇地无冲突？因为 33 和 32 互质：lane <code>i</code> 访问 word <code>33i</code>，
              落在 bank <code>33i mod 32 = i</code>——32 个 lane 恰好均匀铺满 32 个 bank。这正是{' '}
              ...
              冲突消失。代价不过 128 B 的片上空间，换回最多 32 倍的速度——GPU 编程里性价比最高的一行改动。
            </>,
          )}
        </p>
        <Quiz
      ...
      >
        <p>
          {t(
            <>
              ...
              shared among all resident threads — the more one thread uses, the fewer threads the SM can keep
              alive at once (foreshadowing the occupancy chapter). The subtler trap is the{' '}
              ...
              of cycles per access once it spills; that's exactly how a performance cliff appears (compile with{' '}
              <code>-Xptxas -v</code> to see the spill byte count).
            </>,
            <>
        ...
        <p>
          {t(
            <>
              <strong>Constant memory</strong>: a 64 KB read-only region with a dedicated on-chip cache. Its
              trump card is the <strong>broadcast</strong> — when a whole warp reads the same address, one hit
              feeds all 32 threads at near-register speed; but if the addresses within the warp all differ, the
              access serializes and it's actually slower. Good for small "everyone reads the same copy" data like
              convolution kernel coefficients or hyperparameters. <strong>L2 cache</strong> is the device-wide
              transparent cache (40 MB on A100, 50 MB on H100); every global access passes through it, beyond
              your control either way — but it explains many "performance is mysteriously good when the data is
              small" phenomena.
            </>,
            <>
              <strong>Constant memory</strong>：64 KB 的只读区域，配专用片上缓存。它的杀手锏是<strong>广播</strong>——
        ...
        <p>
          {t(
            <>
              ...
              <code>cudaMallocHost</code> can be DMA'd directly — not just faster to copy, but the prerequisite
              for asynchronous <code>cudaMemcpyAsync</code>. Only with it can a PCIe transfer overlap with kernel
              compute on the timeline — the basic skill for hiding data-movement latency in the later inference-
              serving chapter.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              addresses in the same bank serialize N-fold — one padding column (<code>[32][33]</code>) fixes it.
            </>,
            <>
