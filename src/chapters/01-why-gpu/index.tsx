  ...
  return (
    <>
      <p>
        {t(
          <>
            Training a frontier large language model burns through roughly 1×10²⁵{' '}
            <Term t="FLOPs">
            ...
            mistake, it would take tens of thousands of years. The actual training ran on tens of thousands of
            GPUs for about three months. Same silicon, same transistor process, single-chip power in the same
            ballpark, yet throughput differs by five or six orders of magnitude. The counterintuitive part is that
            the gap has nothing to do with GPU cores being faster. They aren&apos;t: a single GPU core is slower
            and dumber than a CPU core. This chapter writes no CUDA at all. It hammers home one point — what kind
            of machine a GPU actually is, where its speed comes from, and when it loses.
          </>,
          <>
            训练一个前沿大语言模型，大约要消耗 1×10²⁵ 次浮点运算（
            <Term t="FLOPs">
              Floating-point Operations，浮点运算次数。注意与 FLOPS（每秒浮点运算次数，衡量算力）区分：前者是
              「工作量」，后者是「干活速度」。
            </Term>
            ，floating-point operations）。这个数字大到失去直觉，换算一下就清楚了。一颗顶级服务器 CPU
            的峰值算力约为几个 TFLOPS（每秒万亿次），就算它满负荷、不眠不休、永不出错，也要连续算上几万年；
            而真实的训练，是数万张 GPU 跑了大约三个月。同样的硅片、同样的晶体管工艺、单芯片功耗也在同一个
            数量级，吞吐却差出五六个数量级。最反直觉的地方在于：这差距跟「GPU 核心更快」毫无关系。它们并不更快，
            单个 GPU 核心比 CPU 核心更慢、更笨。这一章一行 CUDA 都不写，只把一件事讲透：GPU
      ...
      >
        <p>
          {t(
            <>
              The CPU is built to <strong>finish a single task as fast as possible</strong>, which means
              minimizing latency. To get there it packs the chip with circuitry that does no arithmetic at all:
              multi-level caches, tens of megabytes of them, to hide how slow memory is; a branch predictor that
              bets which way an if/else will go and pockets dozens of cycles when it bets right; an out-of-order
              engine that tears instructions apart and reorders them so whichever has its data ready goes first;
              plus speculative execution, prefetchers, and the rest. The whole machine exists to keep those few
              cores from <strong>ever stalling to wait</strong>. The cost is blunt. On a modern flagship CPU die,
              the ALUs (Arithmetic Logic Units) that actually do the arithmetic occupy only a small patch, and the
              vast majority of the area goes to cache and control logic.
            </>,
            <>
              CPU 要做的是<strong>把单个任务尽快做完</strong>，也就是把延迟（latency）压到最低。为此，
              它在芯片上堆了大量「不直接做计算」的电路：动辄几十 MB 的多级缓存（cache）用来掩盖内存的慢；
              分支预测器（branch predictor）赌 if/else 会往哪边走，赌对了就白赚几十个周期；乱序执行
              （out-of-order execution）引擎把指令拆开重排，谁的数据先就绪谁先上；还有投机执行、预取器等等。
              这一整套机器只为一件事：让那少数几个核心<strong>永远不要停下来等</strong>。代价很直白。在一颗
              现代旗舰 CPU 的 die（裸片）上，真正做算术的 ALU（Arithmetic Logic Unit，算术逻辑单元）
              只占很小一块，面积的绝大部分都给了缓存和控制逻辑。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              The GPU answers the opposite way. Its goal is to <strong>maximize total tasks completed per unit
              time</strong>, which is throughput. A single core is allowed to be slow and dumb (no reordering,
              almost no branch prediction, a clock only a third of the CPU&apos;s) as long as there are a lot of
              them. An H100 has 16,896 FP32 cores spread across 132 SMs (Streaming Multiprocessors), and all the
        ...
        <p>
          {t(
            <>
              ...
              A delicate operation, where every step depends on the last and the plan changes on the fly with the
              ...
              down, even when each worker tightens a screw at a fraction of the surgeon&apos;s speed. The analogy
              also spells out the GPU&apos;s <strong>precondition</strong>: the work has to split into a great many
              independent pieces, run at the same time, never waiting on each other and never constantly swapping
              intermediate results. The moment that precondition fails, the GPU&apos;s advantage evaporates, which
              we&apos;ll verify with an experiment in a moment.
            </>,
            <>
              一个常用的类比：CPU 是一位顶尖外科医生，GPU 是一万个流水线工人。一台精密手术，每一步都依赖
              上一步的结果，还要随时根据出血情况改方案，只能交给医生，一万个工人围上来反而碍事。可一旦活儿变成
              「给一百万个零件各拧一颗螺丝」，工人军团完胜，哪怕每个工人拧一颗螺丝的速度只有医生的几分之一。
              这个类比也顺带交代了 GPU 的<strong>适用前提</strong>：任务得能拆成大量彼此独立的小块，可以
              同时做、互相不等待、不需要频繁交换中间结果。一旦前提不成立，GPU 的优势会瞬间消失，这点我们马上
              用实验验证。
            </>,
          )}
        </p>
        ...
        <Callout type="insight" title={t('A GPU is not a “faster CPU”', 'GPU 不是「更快的 CPU」')}>
          <p>
            {t(
              <>
                The two optimize entirely different quantities. The CPU minimizes <strong>the time to finish a
                single task</strong> (latency); the GPU maximizes <strong>the total tasks finished per unit
                time</strong> (throughput). That makes &ldquo;how many times faster is a GPU than a CPU&rdquo; an
                ill-posed question. First ask whether your problem contains tens of thousands of small chores that
                can run at once without waiting on each other. If it does, the GPU is two orders of magnitude
                faster; if it doesn&apos;t, the GPU may be slower than your laptop.
              </>,
              <>
                两者优化的根本就不是同一个量：CPU 最小化<strong>单个任务的完成时间</strong>（延迟），GPU 最大化
                <strong>单位时间完成的任务总量</strong>（吞吐）。所以「GPU 比 CPU 快多少倍」这个问题本身就不完整。
                先问你的问题里有没有几万个能同时做、互不等待的小活。有，GPU 快两个数量级；没有，GPU
                可能比你的笔记本还慢。
              </>,
            )}
          </p>
      ...
      >
        <p>
          {t(
            <>
              ...
              handing work to the GPU costs time in itself. All tasks are independent. Drag the task count down to
              the minimum of 64 and see who wins, then push it to a million and watch how wide the gap gets. Then
              crank the GPU per-core slowdown all the way to 32× and see whether &ldquo;many cores&rdquo; can hold
              the line against &ldquo;slow cores.&rdquo;
            </>,
            <>
              ...
              差距能拉到多大；接着把 GPU 单核慢倍数拉满到 32×，看看「核多」撑不撑得住「核慢」。
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              (2048 ÷ 8 ÷ slowdown). And notice the GPU&apos;s progress bar jumps a notch at a time: 2048 cores
              start together and finish together, advancing one &ldquo;wave&rdquo; at a time. That notion arrives
        ...
        <p>
          {t(
            <>
              One hidden condition deserves saying out loud: in this race every task is <strong>independent</strong>.
            ...
            <>
              还有一个隐藏条件得说破：这场赛跑里所有任务<strong>彼此独立</strong>。一旦程序里出现无法并行的
              串行部分，加速比就有了天花板，这就是 Amdahl 定律（Amdahl&apos;s Law）。设串行部分占总工作量的
        ...
        <p>
          {t(
            <>
              Even if just 5% of the work must run serially, the speedup can never exceed 20×, no matter how many
              tens of thousands of cores you pile on. That is why the first question about &ldquo;can I use a
              GPU&rdquo; is always &ldquo;how much parallelism does my problem have.&rdquo; The next section looks
              at deep learning&apos;s answer.
            </>,
            <>
              哪怕只有 5% 的工作必须串行执行，无论你堆多少万个核心，加速比也不可能超过 20×。这就是为什么
              「能不能用 GPU」的第一个问题永远是「你的问题里有多少并行度」。下一节就去看深度学习的答案。
            </>,
          )}
        </p>
        <Quiz
          ...
          options={[
            ...
            },
            {
              ...
              correct: true,
              explain: t(
                'Correct. This is a hard serial dependency: until step i finishes, step i+1 doesn&apos;t even know where its data lives, so out of tens of thousands of cores only one ever has work and the rest stand and watch. Add the GPU&apos;s slower single core and weaker branch handling on top, and this kind of pointer-chasing task actually runs slower on a GPU than on a CPU.',
                '对。这是强串行依赖：第 i 步不做完，第 i+1 步连数据在哪都不知道，几万个核里始终只有一个有活干，' +
                  '其余全部围观。再叠加 GPU 单核本身更慢、分支处理更弱，这类指针追逐（pointer chasing）任务在 GPU 上反而比 CPU 慢。',
              ),
            },
            {
        ...
        <p>
          {t(
            <>
              ...
              vector add is a million naturally independent little tasks. On a CPU we habitually write it as a
              loop:
            </>,
            <>
        ...
        <p>
          {t(
            ...
            <>
              ...
              要做的活是什么」。把循环体抠出来，写成一个工人的工作说明书，然后雇 n 个工人同时开工。循环就此消失，
        ...
        <p>
          {t(
            <>
              ...
              split into 16.7 million mutually independent dot products. That much parallelism is more than enough
              to feed tens of thousands of cores.
            </>,
            <>
              ...
              1.4×10¹¹ 次运算，拆成 1670 万个互不依赖的点积。这点并行度，喂饱几万个核心绰绰有余。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              The final inference follows on its own: the body of a neural network is just stacked matrix
              multiplies. A fully connected layer is a matmul. The heart of attention is a matmul. Convolution can
              be cast as one too. A single forward pass of a large model is essentially a few hundred big matmuls
              run back to back, each carrying tens of millions of degrees of parallelism inside. (Frontier models
              add wrinkles. Mixture-of-Experts routes each token through only a slice of the weights; attention has
              shrunk its key/value cost with grouped-query attention. Every one of those wrinkles still bottoms out
              in matmuls.) This class of problem, where the parallelism shows up on its own with no
              painstaking reworking, even has a name: embarrassingly parallel. Deep learning and the GPU didn&apos;t
              so much choose each other as make each other. The GPU finally got a workload that fills all its cores,
              and deep learning finally got a machine that grinds through its computation in a reasonable amount of
              time.
            </>,
            <>
              最后一步推论就水到渠成了：神经网络的主体就是矩阵乘的堆叠。全连接层是矩阵乘，注意力机制
              （attention）的核心是矩阵乘，卷积也能化成矩阵乘。一次大模型的前向传播，本质上是几百个大矩阵乘
              排队执行，每一个内部都携带千万级的并行度。（前沿模型加了些花样：混合专家（Mixture-of-Experts）
              让每个 token 只经过一部分权重，注意力则用分组查询注意力（GQA）压低了 key/value 的开销，但这些
              花样最终都还是落到矩阵乘上。）这类不需要费心改造、并行度自己送上门的问题，有个专门的称呼：
              embarrassingly parallel（「不好意思地并行」，意为并行得毫不费力）。深度学习和 GPU 的关系，与其说
              是谁选了谁，不如说是互相成就：GPU 等到了能填满它所有核心的负载，深度学习等到了能在合理时间内
              把它的计算量算完的机器。
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              always sum to 100%: drag any one and the other two scale proportionally, exactly as in real chip
              design, where area given to cache can no longer go to ALUs. The two readouts on the right are the
              ...
              rate far less than going from 0% to 20% did, and branch prediction behaves the same way: once
              accuracy nears 98%, every further point costs multiplying transistors. The <strong>throughput
              score</strong> is strictly proportional to ALU count. One more core, one more unit of output, crude
              and linear with no falloff.
            </>,
            <>
              这个实验把 SEC 01 的那张静态图变成可以拖的。三个滑杆的总和恒为 100%：拖动任何一个，另外两个会
              按比例缩放，就像真实的芯片设计，面积给了缓存就不能再给 ALU。右侧两个读数对应两种设计哲学的
        ...
        <p>
          {t(
            <>
              ...
              everything into control and cache and the single-thread score still only approaches an asymptote,
              because diminishing returns lock the ceiling. The two preset buttons give the typical trade-offs of
              the two real-world paths. Watch one detail: the &ldquo;Typical GPU&rdquo; preset does not put cache
              at 0%. A real GPU keeps a register file, shared memory, and a modest L2, and their roles differ a lot
              from CPU cache (closer to a scratchpad the programmer manages by hand). That&apos;s a centerpiece of
              Chapters <ChapterLink n={2} label="2" /> and <ChapterLink n={4} label="4" />.
            </>,
            <>
              试试两个极端。全部砸给 ALU，吞吐分拉满，但单线程分惨不忍睹，这台机器跑串行代码会被手机吊打；
              全部砸给控制和缓存，单线程分也只能逼近渐近线，因为收益递减锁死了天花板。两个预设按钮给出的正是
              现实中两条路线的典型取舍。留意一个细节：「典型 GPU」预设里缓存并不是
      ...
      >
        <p>
          {t(
            <>
              All chapter we&apos;ve assumed that once a task reaches a core it can compute. In the real world the
              data sits in memory, and memory is far slower than arithmetic. That gap widens year over year:
              compute roughly more than doubles every two years while memory bandwidth and latency improve much
              more slowly. This is the famous memory wall, and David Patterson distilled its more painful half into
              &ldquo;latency lags bandwidth.&rdquo; Bandwidth can still be widened by piling on parallelism;
              latency is nearly incurable.
            </>,
            <>
              本章我们一直假设「任务到了核心手里就能算」。可真实世界里数据躺在内存里，而内存比算术慢得多，
              而且这个差距还在逐年拉大：算力大约每两年翻一倍以上，内存带宽和延迟的改善却慢得多。这就是著名的
              内存墙（memory wall）。David Patterson 把其中更扎心的那半句总结为「延迟落后于带宽」
              （latency lags bandwidth）：带宽还能靠堆并行做宽，延迟却几乎无药可医。
            </>,
          )}
        </p>
        <p>
          {t(
            <>
              ...
              Bandwidth Memory) delivers about 1.9–2.0 TB/s, the H100&apos;s HBM3 about 3.35 TB/s, and the
              Blackwell generation widens the pipe again. For comparison, a desktop CPU&apos;s dual-channel DDR5
              manages only around 100 GB/s, a 20–30× difference. But wide is not fast. Look at the latency
              magnitudes across the storage hierarchy (in GPU core clock cycles, all approximate):
            </>,
            <>
              GPU 的应对是给自己配上极宽的内存：A100 的 HBM2e（High Bandwidth Memory，高带宽内存）提供约
              1.9～2.0 TB/s 的带宽，H100 的 HBM3 约 3.35 TB/s，到 Blackwell 这一代管子又拓宽了一截。作为对比，
              桌面 CPU 的双通道 DDR5 大约只有 100 GB/s，差了 20～30 倍。但「宽」不等于「快」。看一眼各级存储
              的延迟数量级（以 GPU 核心时钟周期计，均为约数）：
            </>,
          )}
        </p>
        ...
        <p>
          {t(
            <>
              ...
              just sat there waiting. Of course the GPU doesn&apos;t sit and wait. Rather than build a bigger cache
              like the CPU, it throws a huge number of threads at the problem and lets them cover for one another:
              while one batch of threads waits on data, the hardware instantly switches to another batch and
              computes in the meantime, hiding latency with parallelism. How this latency-hiding mechanism works,
              and how many threads it takes to hide the latency, is the core of <ChapterLink n={2} />. How to
              organize access patterns so that 2 TB/s of bandwidth is actually saturated rather than squandered on
              scattered accesses is the entire subject of <ChapterLink n={4} />. Remember this chapter&apos;s
              conclusion: <strong>compute solves &ldquo;computing fast&rdquo;; the memory system decides
              &ldquo;whether you get fed,&rdquo; and the latter is the bottleneck for most real programs.</strong>
            </>,
            <>
              等一次显存的时间，核心足够做几百次乘加，前提是它干等。GPU 当然不会干等。它的解法不是像 CPU
              那样修更大的缓存，而是甩出海量线程让它们互相掩护：一批线程在等数据，硬件就立刻切到另一批先算着，
              用并行度把延迟「藏」起来。这套延迟隐藏（latency hiding）机制怎么运转、需要多少线程才藏得住，是
              <ChapterLink n={2} />的核心；而怎么组织访存模式让那 2 TB/s 的带宽真正跑满、不浪费在零碎的访问上，
              是<ChapterLink n={4} />的全部内容。记住本章的结论：<strong>算力解决「算得快」，内存系统决定
              「喂不喂得饱」，而后者才是大多数真实程序的瓶颈。</strong>
            </>,
          )}
        </p>
      </Section>
          ...
          <li>
            {t(
              <>
                ...
                &ldquo;getting fed&rdquo; is harder than &ldquo;computing fast.&rdquo; That&apos;s the theme of{' '}
                <ChapterLink n={2} /> (latency hiding) and <ChapterLink n={4} /> (the memory hierarchy).
              </>,
              <>
                <strong>内存墙在前方</strong>：算力增长快于内存带宽，「喂得饱」比「算得快」更难，这正是<ChapterLink n={2} />
