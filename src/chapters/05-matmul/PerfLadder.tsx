...
export function PerfLadder() {
  ...
  const LEVELS: { name: string; pct: number; tone: string; bar: string }[] = [
    ...
    { name: t('coalesced access', '合并访存'), pct: 8, tone: 'text-ink2', bar: 'bg-amber/45' },
    { name: 'shared tiling', pct: 40, tone: 'text-ink2', bar: 'bg-amber/60' },
    { name: t('register tiling', '寄存器 tiling'), pct: 70, tone: 'text-ink2', bar: 'bg-cyan/55' },
    { name: t('vectorize + double buffer', '向量化+双缓冲'), pct: 85, tone: 'text-ink2', bar: 'bg-cyan/75' },
    { name: 'cuBLAS / Tensor Core', pct: 100, tone: 'text-volt', bar: 'bg-volt/70' },
  ]
  return (
    <Figure
      caption={t(
        'SGEMM optimization ladder: each kernel level relative to cuBLAS (FP32, large matrices; figures synthesized from public reproductions such as Simon Boehm’s — orders of magnitude, for intuition only)',
        'SGEMM 优化阶梯：各级 kernel 相对 cuBLAS 的大致性能（FP32、大矩阵、数据综合自 Simon Boehm 等公开复现实验，量级仅供直觉参考）',
      )}
    >
