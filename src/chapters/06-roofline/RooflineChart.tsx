...
const KERNELS: KernelPt[] = [
  {
    ...
    frac: 0.85,
    blurb:
      '逐元素操作：每搬一个字节只摊到零点几个 FLOP。不管怎么调 block size，它都贴死在带宽斜线上——典型的纯带宽生意。',
  },
  {
    id: 'gemv',
    name: 'GEMV',
    ...
    frac: 0.8,
    blurb:
      '矩阵×向量：矩阵的每个元素从显存读进来只参与 2 次运算就被丢掉，零复用。batch=1 的 LLM 线性层本质上就是一排 GEMV。',
  },
  {
    id: 'attn',
    name: 'attn-decode',
    ...
    frac: 0.6,
    blurb:
      '解码阶段一条 query 扫整个 KV cache：每个 K/V 元素只用一两次，AI ≈ 1~2。这就是 LLM 推理 memory-bound 的根源，后面三章的主角。',
  },
  {
    id: 'sgemm',
    name: '小 GEMM 64³',
    ...
    frac: 0.5,
    blurb:
      '矩阵太小，复用有限：BF16 方阵的 AI ≈ n/3，n=64 只有 21 FLOP/B，离 A100 的 ridge（164）差一个数量级。还没起跑就被带宽掐住。',
  },
  {
    id: 'lgemm',
    name: '大 GEMM 2048³',
    ...
    side: -1,
    blurb:
      'n=2048 时 AI ≈ n/3 ≈ 680，远超任何卡的 ridge，稳稳压在算力平台上——Tensor Core 终于吃饱。矩阵要够大，算力才有意义。',
  },
]
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* FP32 屋顶（虚线） */}
        <line x1={px(ridge32)} y1={py(hw.fp32)} x2={VW - MR} y2={py(hw.fp32)} stroke="var(--color-ink3, #6b7693)" strokeWidth={1.2} strokeDasharray="5 5" opacity={0.7} />
        <text
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_1.3fr]">
          ...
          {hoveredK ? (
            <>
              <div className="mb-1 flex items-baseline gap-3">
                <span className="font-mono text-sm text-cyan">{hoveredK.name}</span>
                <span className="font-mono text-[11px] text-ink3">
                  AI ≈ {fmtAI(hoveredK.ai)} FLOP/B · 屋顶 {attainable(hoveredK.ai, hw.bw, hw.tensor).toFixed(attainable(hoveredK.ai, hw.bw, hw.tensor) < 10 ? 2 : 0)} TFLOPS @ {hw.name}
                </span>
              </div>
              {hoveredK.blurb}
            </>
            ...
            <>
              <span className="microlabel mb-1 block">预置 KERNEL</span>
              悬停或点按图中的青色点，看每类 kernel 为什么落在那里。注意它们几乎都挤在 ridge 左边
              ——日常 kernel 里 compute-bound 反而是稀有物种。
            </>
          )}
        </div>
        ...
        <div className="rounded-md border border-volt/30 bg-volt/[0.04] px-4 py-3.5">
          <div className="mb-2 flex flex-wrap items-end gap-x-6 gap-y-2">
            <Stat label="算术强度" value={fmtAI(user.ai)} unit="FLOP/B" tone="volt" size="sm" />
            <Stat label="实测性能" value={uTf < 10 ? uTf.toFixed(2) : uTf.toFixed(0)} unit="TFLOPS" tone="ink" size="sm" />
            <Stat label="此处屋顶" value={uRoof < 10 ? uRoof.toFixed(2) : uRoof.toFixed(0)} unit="TFLOPS" tone={memSide ? 'amber' : 'volt'} size="sm" />
            <Stat
              label="距屋顶 = 优化空间"
              value={`×${gap.toFixed(1)}`}
          ...
          <p className="text-[13px] leading-[1.9] text-ink2">
            <span className={`mr-2 font-mono text-[11px] tracking-wider ${memSide ? 'text-amber' : 'text-volt'}`}>
              {memSide ? '◀ MEMORY-BOUND 区' : 'COMPUTE-BOUND 区 ▶'}
            </span>
            屋顶 = {memSide ? `AI × ${hw.bw} TB/s` : `峰值 ${hw.tensor} TFLOPS`}。{advice}
          </p>
        </div>
      </div>
    </Widget>
