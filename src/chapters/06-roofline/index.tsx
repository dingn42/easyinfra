  ...
  return (
    <>
      <p>
        {t(
          <>
            "This kernel is too slow." Hand a beginner that ticket and the first move is to open the code and scan
            line by line; hand it to a veteran and the first move is to ask a yes/no question:{' '}
            <strong>is it starved for compute, or starved for data?</strong> The first means the math units can't keep
            up; the second means the memory system can't feed them. The two diseases share every symptom (everything is
            just "slow") but the cures are exact opposites — prescribe the wrong one and you can optimize for a week
            while performance doesn't budge. The Roofline model compresses that diagnosis into a single log-log plot:
            the x-axis is a program's "constitution," the y-axis is how fast it can possibly run, and any kernel pinned
            onto it reveals its bottleneck at a glance. This chapter builds that plot from scratch, then shows you how to
            pin your own kernels onto it with a real profiler.
          </>,
          <>
      ...
      <Section
        ...
        lead={t(
          'One ratio defines the program’s constitution, another defines the machine’s temperament — diagnosis is just comparing the two.',
      ...
      >
        ...
        <p>
          {t(
            <>
              ...
              two limits cross is called the{' '}
              <Term
                t={
                  <>
              ...
              >
                ...
                compute and bandwidth at the same time; it's the point on the Roofline where the slanted line meets the
                flat plateau, and the dividing line between memory-bound and compute-bound.
              </Term>
              :
            </>,
            <>
        ...
        <p>
          {t(
            <>
              AI below the ridge means time is dominated by moving data — that's{' '}
              <strong className="text-amber">memory-bound</strong>; above the ridge, time is dominated by compute —
              that's <strong>compute-bound</strong>. Note the division of labor between the two numbers: AI is a property
              of the <strong>program</strong>, fixed by the algorithm and its implementation, independent of which card
              it runs on; the ridge is a property of the <strong>machine</strong>, independent of your code. Diagnosing a
              bottleneck is just laying these two numbers side by side and comparing them. The ridges of a few common
              cards:
            </>,
            <>
        ...
        <p>
          {t(
            <>
              See the pattern? On the same A100, the FP32 CUDA-Core ridge is only about 10 — most programs are
              "qualified" to saturate FP32 compute; but switch to the BF16 Tensor Core and the ridge rockets to 164, and
              the programs that can clear that bar become a precious few.{' '}
              <strong>Tensor Cores lifted compute 16×, but bandwidth stayed put, so "starved for data" became the
          ...
          <p>
            {t(
              <>
                ...
                usually assume the ideal case (each datum crosses DRAM exactly once), which gives an upper bound on AI;
      ...
      <Section
        ...
        lead={t(
          'Once you count the numerator and denominator straight, AI is grade-school division — the hard part is knowing what to count.',
          '数清楚分子分母，AI 就是个小学除法——难的是知道该数什么。',
        )}
      >
        <p>
          {t(
            <>
              Take the most important case, GEMM (general matrix multiply, M×K times K×N): each output element does K
              multiply-adds, for 2MNK FLOPs total; in the ideal case the three matrices each cross memory once, so the
              byte count is (MN + MK + KN)·b, where b is the bytes per element (2 for BF16). Tidy it up and you get a
              rather flavorful form:
            </>,
            <>
        ...
        <p>
          {t(
            <>
              The right side is the harmonic mean of M, N, K — same shape as resistors in parallel, where{' '}
              ...
              its constitution. This is the mathematical root of "matrices have to be big enough to saturate compute."
              Flip it around: even with M and N both at 8192, the moment K=16 the short edge pins AI to the floor. Verify
              it yourself with the calculator below:
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              <strong>vector add, SAXPY, and GEMV have constant AI</strong> — numerator and denominator both scale with
              the data size, so scaling up never changes the constitution; they're doomed to be memory-bound. Attention
              score's AI is set jointly by sequence length and head dimension; only GEMM can keep improving its
              constitution by growing larger. This is why "batch small ops into one big matmul" is the universal motif of
              GPU performance work — from cuBLAS batched GEMM to LLM-serving continuous batching, it's the same trick at
              heart.
            </>,
            <>
        ...
        options={[
          {
            ...
            correct: true,
            explain: t(
              'In FP16 that’s 1 FLOP / 6 B ≈ 0.17; in FP32, 1/12 ≈ 0.08 — whatever the precision or counting convention, it hovers between 0.1 and 0.3, two to three orders of magnitude away from any ridge. Running this kernel, the GPU spends nearly all its time moving data while the add units just watch.',
          ...
          },
          {
            ...
            explain: t(
              'Both FLOPs and bytes scale with N, so N cancels in the ratio — an elementwise op’s constitution is constant, and scaling up can’t save it. That’s precisely what separates it from GEMM.',
      ...
      <Section
        ...
        lead={t(
          ...
          'x 轴是体质（AI），y 轴是可达性能，两道屋顶罩在头上——所有 kernel 在这张图上无所遁形。',
        )}
      >
        <p>
          {t(
            <>
              ...
              performance on the y-axis (log scale). The left half is a slanted line — performance = AI × bandwidth,
              where every bump in a byte's "value" lifts performance proportionally; past the ridge it gets capped by
              compute and flattens into a horizontal plateau. Slope plus plateau looks like a roof, which is how the
              model got its name. A kernel is a point on this plot: its x-coordinate is set by the algorithm, its
              y-coordinate is measured performance —{' '}
              <strong>the point always sits below the roof, and the vertical distance from point to roof is the headroom
              you haven't cashed in yet</strong>.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              Two things are worth chewing on repeatedly. First, four of the five preset points cluster to the left of
              the ridge — among everyday kernels, compute-bound is the rare species and "starved for data" is the norm.
              Second, switch to the H100: bandwidth grew about 1.76× over the A100, but compute grew about 3.2×, pushing
              the ridge from 164 right to about 295 —{' '}
              <strong>the newer the card, the more surplus compute it has, and the fewer programs can saturate it</strong>.
              Hardware's trajectory has already spelled it out: future performance engineering will, in all likelihood,
              be a fight against bandwidth.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              stretch of roof equals AI × BW and has nothing to do with peak compute. There are only two roads to going
              faster. First, <strong>move right (raise AI)</strong>: (1)<strong> kernel fusion</strong> — collapse a
              chain like add → mul → GELU into one kernel so intermediate results stay in registers and never hit DRAM,
              slashing the DRAM byte count by two-thirds; (2){' '}
              <strong>tiling for reuse</strong> — last chapter's GEMM shared-memory blocking, which at heart makes each
              byte from DRAM serve more ops; (3){' '}
              <strong>quantization</strong> — FP16 to INT8/INT4, halving and halving again the bytes for the same data,
              doubling AI in place. Second, <strong>lift the roof itself</strong>: swap in a card with faster HBM, or move
              hot data into a faster storage tier (L2, shared memory — effectively drawing a steeper slope against a
              nearer "DRAM").
            </>,
            <>
        ...
        <p>
          {t(
            <>
              ...
              occupancy so the SMs always have work, and kill warp stalls. And if the point hugs neither roof but hangs
              in mid-air, that's a more elementary problem: uncoalesced accesses, branch divergence, or kernels so tiny
              that launch overhead dominates — fix utilization first, then talk direction.
            </>,
            <>
        ...
        options={[
          {
            ...
            correct: true,
            explain: t(
              'Three "read + write back" round-trips become one, cutting DRAM bytes to about 1/3 and tripling AI — the whole point shifts right on the Roofline. This is the core payoff of fusing elementwise chains in torch.compile and Triton.',
          ...
          },
          {
            ...
            explain: t(
              'Bandwidth is already at 90%; the bottleneck is HBM throughput, not latency hiding — more warps just lengthen the queue. Occupancy is the cure when the point is "far from the roof," not when it’s "hugging the slope."',
          ...
          },
          {
            ...
            explain: t(
              'Elementwise has no matmul structure, so Tensor Cores simply can’t apply; and the bottleneck is bandwidth anyway — no amount of compute can push the roof in the slope region.',
          ...
          },
          {
            ...
            explain: t(
              ...
              '斜线区的屋顶 = AI × BW，与峰值算力无关——这是四个选项里最贵且唯一完全无效的方案。',
            ),
          },
        ]}
      />
      ...
      >
        <p>
          {t(
            <>
              NVIDIA's performance tools have a clear division of labor, and remembering this order saves you half the
              detours: <strong>Nsight Systems first, then Nsight Compute</strong>. Nsight Systems gives you the timeline
              of the whole program — what the CPU is doing, when each kernel launches, whether memory copies overlap with
              compute. A lot of "the GPU is slow" cases crack at this layer: the GPU is actually idle, time is lost in the
              gaps between kernels (CPU submitting too slowly, needless synchronization), or H2D/D2H copies sit serially
              in the middle of compute. For that disease, optimizing the kernel itself is barking up the wrong tree.
            </>,
            <>
        ...
        <p>
          {t(
            <>
              Only after confirming the time really is being spent inside some kernel does Nsight Compute's turn come —
              the microscope for a single kernel. It replays the target kernel dozens of times, samples every hardware
              counter, and even ships with a built-in Roofline analysis page: it computes your AI and measured FLOPS
              automatically and plots the point under the roof. Even if you don't have a GPU environment on hand right
              now, it's worth burning the reading of these few metrics into your brain — they're the common language of
          ...
          {t(
            <>
              ...
              <Term t={<>occupancy</>}>
                ...
                your "budget for hiding latency": the more warps, the easier it is to find another warp to step in while
                one is waiting on memory. But higher isn't always better — enough is enough.
              </Term>{' '}
              is the easiest to misread: it isn't performance itself but the budget for latency hiding, and "occupancy is
              high yet neither bandwidth nor compute is saturated" is exactly the sign that the problem lies elsewhere.
              Build a habit: with any profiler report in hand, find DRAM% and SM% first, plot the kernel onto the
              Roofline in your head, then decide whether to push right or up — that two-step move is the watershed
              between a performance engineer and a "tune-by-superstition" hacker.
            </>,
            <>
              其中{' '}
          ...
          <li>
            {t(
              <>
                ...
                <strong>the thinnest dimension calls the shots</strong> — the bigger the square matrix, the better its
                constitution.
              </>,
              <>
